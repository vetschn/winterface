...
template <>
inline constexpr const char *idstr<std::vector<std::string>>() noexcept {
  return "STRINGS";
}
  ...
  try {
    ...
    while (std::getline(file, line)) {
      ...
      if (pk.find(key) != pk.end())
        throw(std::invalid_argument("duplicate key \'" + i->str() + "\'"));
...
struct aux_parser {
 ...
 public:
   ...
   * version of this struct which must inherit from this class. The parsing
   * process is defined in the constructor, who will then call all the other
   * constructors in the inheritance hierarchy from top to bottom, thus parsing
   * all the keys. Types of keys supported can be extended by writing a parse_
   * for a new type.
   */
  struct parseKey_ {
   public:
     ...
     * @param i regex iterator pointing to the first token of the line being
     * parsed
     * @param e regex iterator past the end of the tokens in the line being
     * parsed
     */
    explicit parseKey_(aux_parser &p, const uint32_t key, std::ifstream &file,
                       size_t &lcnt, std::sregex_token_iterator i,
    ...
    template <class T>
    inline void parse(T &inp, std::ifstream &file, size_t &lcnt,
                      std::sregex_token_iterator &i,
                      const std::sregex_token_iterator &e) {
      parse_(inp, file, lcnt, i, e);
      ++rcnt_;
    }
    //! parse wrapper with rcnt increment for two arguments
    template <class T>
    inline void parse(T &inp, const size_t n, std::ifstream &file, size_t &lcnt,
                      std::sregex_token_iterator &i,
                      const std::sregex_token_iterator &e) {
      parse_(inp, n, file, lcnt, i, e);
      ++rcnt_;
    }
    //! parse wrapper with rcnt increment for three arguments
    template <class T>
    inline void parse(T &inp, const bool treat, const size_t n,
                      std::ifstream &file, size_t &lcnt,
                      std::sregex_token_iterator &i,
                      const std::sregex_token_iterator &e) {
      parse_(inp, treat, n, file, lcnt, i, e);
      ++rcnt_;
    }
   ...
   protected:
     ...
     * past the key and the optional '=' to where the data is.
     */
     ...
     * parsed
     */
    inline static void parse_(std::string &inp, std::ifstream &file,
                              size_t &lcnt, std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
      bitch_(i, e);
     ...
     * parsed
     */
    inline static void parse_(std::vector<std::string> &inp,
                              std::ifstream &file, size_t &lcnt,
                              std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
      bitch_(i, e);
     ...
     * parsed
     */
    inline static void parse_(double &inp, std::ifstream &file, size_t &lcnt,
                              std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
      bitch_(i, e);
      assert(file || true || lcnt);
     ...
     * parsed
     */
    inline static void parse_(size_t &inp, std::ifstream &file, size_t &lcnt,
                              std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
      bitch_(i, e);
      assert(file || true || lcnt);
     ...
     * parsed
     */
    inline static void parse_(bool &inp, std::ifstream &file, size_t &lcnt,
                              std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
      bitch_(i, e);
      assert(file || true || lcnt);
      if (i->str().front() == 't' || i->str().front() == 'T') {
        inp = true;
        return;
      }
      if (i->str().front() == 'f' || i->str().front() == 'F') {
        inp = false;
        return;
      }
      inp = std::stoi(*i);
     ...
     * parsed
     */
    inline static void parse_(std::vector<double> &inp, const size_t n,
                              std::ifstream &file, size_t &lcnt,
                              std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
      bitch_(i, e);
     ...
     * parsed
     */
    inline static void parse_(std::vector<double> &inp, std::ifstream &file,
                              size_t &lcnt, std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
     ...
     * parsed
     */
    inline static void parse_(std::vector<size_t> &inp, const bool treat,
                              const size_t n, std::ifstream &file, size_t &lcnt,
                              std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
      bitch_(i, e);
      assert(file || true || lcnt);
     ...
     * parsed
     */
    inline static void parse_(std::vector<size_t> &inp, const size_t n,
                              std::ifstream &file, size_t &lcnt,
                              std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
     ...
     * parsed
     */
    inline static void parse_(std::vector<size_t> &inp, std::ifstream &file,
                              size_t &lcnt, std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
     ...
     * parsed
     */
    inline static void parse_(std::vector<bool> &inp, const size_t n,
                              std::ifstream &file, size_t &lcnt,
                              std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
      bitch_(i, e);
     ...
     * parsed
     */
    inline static void parse_(std::vector<bool> &inp, std::ifstream &file,
                              size_t &lcnt, std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
      parse_(inp, NPOS__, file, lcnt, i, e);
    }
     ...
     * parsed
     */
    inline static void parse_(lm__::fMat &inp, std::ifstream &file,
                              size_t &lcnt, std::sregex_token_iterator &i,
                              const std::sregex_token_iterator &e) {
      // read mat from file
      if (std::distance(i, e)) {
        std::string fileName_;
        parse_(fileName_, file, lcnt, i, e);
 ...
 public:
   ...
   * defined in the constructor, who will then call all the other constructors
   * in the inheritance hierarchy from top to bottom, thus printing all the
   * messages. Additionally, the current contents of a child of aux_parser will
   * be printed. Thus when not initialized, the default values will be printed.
   * The recognized types can be extended by writing additional print_
   * functions.
   */
  struct printHelp_ {
   public:
     ...
     public:
      ...
      inline static constexpr const char *format() noexcept {
        return GREEN__ "key" CYAN__ " [TYPE COLSxROWS]\n" RESET__
                       "purpose of the key and its input\n" CYAN__
                       "value" RESET__ "\n";
      }
   ...
   protected:
    ...
    //! print help tuple
    template <class... Args>
    void printHelpTuple_(std::ostream &os,
                         const std::tuple<Args...> &t) const noexcept {
...
inline std::ostream &operator<<(std::ostream &os,
                                const aux_parser &inp) noexcept {
  aux::printHelp(inp, os);
  return os;
}
//! streaming operator for children of aux_parser
template <class T,
          std::enable_if_t<std::is_base_of<aux_parser, T>::value> * = nullptr>
