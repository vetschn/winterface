...
class mat_cb {
 public:
   ...
   */
  //! data const pointer
  auto data() const noexcept -> const decltype(MT().data()) {
    return mat_.data();
  }
  //! direct column const access
  inline auto cAt(const size_t n) const noexcept -> const
      decltype(MT().cAt(0)) {
    assert(n < N());
    return mat_.cAt(n);
  }
...
class vec_cb {
 ...
 protected:
  ...
  //! push_back operator
  inline void push_back(CB inp) noexcept {
    this->vec_.push_back(std::move(inp));
  }
  //! pop_back operator
 ...
 * columns/elements. The api exposing const access is public. The api exposing
 * all other access is protected. Some of the api is directly exposed such that
 * we can write A.cbegin() instead of A.vec_.cbegin() without having to write a
 * passthrough manually. Some of the functions are such that the operation is
 * done on both containers simultaneously. Both containers must be of the same
 * size at all times.
 */
...
class mat_vec_cb : public mat_cb<MT, AT>, public vec_cb<CT, CB> {
 ...
 protected:
   ...
   */
  virtual inline void reserve(const size_t nN) noexcept {
    mat_cb<MT, AT>::reserve(nN);
    vec_cb<CT, CB>::reserve(nN);
  }
  //! shrink allocated memory to the actual number of elements
  virtual inline void shrink_to_fit() noexcept {
    mat_cb<MT, AT>::shrink_to_fit();
    vec_cb<CT, CB>::shrink_to_fit();
  }
   ...
   */
  virtual inline void resize(const size_t nN) noexcept {
    mat_cb<MT, AT>::resize(nN);
    vec_cb<CT, CB>::resize(nN);
  }
  ...
  inline void push_back(const AT &minp, CB vinp) noexcept {
    mat_cb<MT, AT>::push_back(minp);
    vec_cb<CT, CB>::push_back(std::move(vinp));
  }
  //! pop_back function applied to both containers
  virtual inline void pop_back() noexcept {
    mat_cb<MT, AT>::pop_back();
    vec_cb<CT, CB>::pop_back();
  }
  /** erase function applied to both containers
   * @param n (column) index
   */
  virtual inline void erase(const size_t n) noexcept {
    mat_cb<MT, AT>::erase(n);
    vec_cb<CT, CB>::erase(n);
  }
...
/** Wrapper for a class holding a matrix and vector of the same number of
 * columns/elements. The api exposing const access is public. The api exposing
 * all other access is public. Some of the api is directly exposed such that we
 * can write A.cbegin() instead of A.vec_.cbegin() without having to write a
 * passthrough manually. Some of the functions are such that the operation is
 * done on both containers simultaneously. Both containers must be of the same
 * size at all times.
 */
...
class mat_vec_b : public mat_b<MT, AT>, public vec_b<CT, CB> {
 ...
 public:
   ...
   */
  virtual inline void reserve(const size_t nN) noexcept {
    mat_b<MT, AT>::reserve(nN);
    vec_b<CT, CB>::reserve(nN);
  }
  //! shrink allocated memory to the actual number of elements
  virtual inline void shrink_to_fit() noexcept {
    mat_b<MT, AT>::shrink_to_fit();
    vec_b<CT, CB>::shrink_to_fit();
  }
   ...
   */
  virtual inline void resize(const size_t nN) noexcept {
    mat_b<MT, AT>::resize(nN);
    vec_b<CT, CB>::resize(nN);
  }
  /** push_back operator for both containers
   * @param minp the new column to be inserted at the back back of the matrix
   * @param vinp the new element to be inserted at the back of the vector
   */
  inline void push_back(const AT &minp, CB vinp) noexcept {
    mat_b<MT, AT>::push_back(minp);
    vec_b<CT, CB>::push_back(std::move(vinp));
  }
  //! pop_back function applied to both containers
  virtual inline void pop_back() noexcept {
    mat_b<MT, AT>::pop_back();
    vec_b<CT, CB>::pop_back();
  }
  /** erase function applied to both containers
   * @param n (column) index
   */
