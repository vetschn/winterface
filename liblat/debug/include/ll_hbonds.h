    ...
    inline explicit printHelp_(const ll_wmatching_input &p, std::ostream &os)
      ...
      printHelpTuple_(
          ...
          std::make_tuple(
              ...
              HELP__("wout", p.wout, "name of the wannier90 *.wout file"),
              HELP__("hrdat", p.hrdat, "name of the wannier90 *.hrdat file")
              ...
              HELP__(
                  ...
                  " - cluster: use clustering algorithm to identify clusters "
                  "of\n"
                  "            wannier centers, then use centers of mass as "
                  "positions\n"
                  " - wannier: use the wannier centers themselves, i.e. no "
                  "matching\n"
                  " - 'filename': use positions as defined by a POSCAR file "
                  "'filename'"),
              HELP__("tol", p.tol,
                     "cutoff tolerance for wannier90 hamiltonian data"),
              HELP__("wbl", p.wbl, NPOS__,
                     "wannier functions blacklist, as to which hamiltonian "
                     "entries and centers\n"
                     "should be discarded"),
              HELP__("abl", p.abl, NPOS__,
                     "atomic positions blacklist, as to which atomic centers "
                     "should be discarded"),
              HELP__(
                  "bond_factor", p.bond_factor,
              ...
              HELP__("rm_unmatched", p.rm_unmatched,
                     "bitmask to keep or discard positions without wannier "
                     "centers matched to it:\n"
              ...
              HELP__("balance_matches", p.balance_matches,
                     "try to balance out the number of centers matched to "
...
public:
   ...
   */
  i_i_R_H(const size_t i1, const size_t i2, fMat R,
          std::vector<cMat> H) noexcept
  ...
  inline auto center() noexcept {
    ...
    return (itr == this->ccEnd() || *itr != 0.0) ? this->end()
                                                 : this->begin() + (size_t)itr;
  }
  //! const_iterator to center interaction matrix (at R=0)
  inline auto center() const noexcept {
    const auto itr = std::lower_bound(this->ccBegin(), this->ccEnd(), 0.0);
    return (itr == this->ccEnd() || *itr != 0.0) ? this->cend()
                                                 : this->cbegin() + (size_t)itr;
  }
  ...
  friend inline i_i_R_H conj(const i_i_R_H &inp) noexcept {
    std::vector<cMat> cH;
    std::transform(inp.cbegin(), inp.cend(), cH.begin(),
                   [](const cMat &inp) -> cMat { return lm__::T(inp); });
 ...
 * of 'atomic' orbitals. The number of orbitals for each 'atom' in the unit cell
 * is therefore the number of Wannier functions matched to it. The physics of
 * the interactions between these atoms is described in terms of the Wannier
...
public:
   ...
   * @param cell unit cell holding 'atomic' positions corresponding to the
   * Wannier center distribution
   * @param Wp Wannier centers
   * @param I Wannier matching indices, one vector per 'atomic' position in
   * cell, one index per center in Wp
   * @param hr Wannier90 Hamiltonian as generated by readHr
   * @param transf generic lambda for manipulating interaction matrices
   */
  ll_hbonds(
      ll_cell cell, const fMat &Wp, const wi &I, const R_H &hr,
      const std::function<cMat(cMat &&)> &transf = [](cMat &&inp) -> cMat {
        return std::move(inp);
      }) noexcept;
  /** constructor from generated material and matched Wannier centers.
   * @param cell unit cell holding 'atomic' positions corresponding to the
  ...
  inline ll_hbonds(ll_cell cell, const fMat &Wp, const wi &I, const R_H &hr,
      ...
      : ll_hbonds(std::move(cell), Wp, I, hr, [keep](cMat &&inp) -> cMat {
          return keep(inp) ? inp : cMat();
        }) {}
  /** constructor from generated material and matched Wannier centers.
   * @param cell unit cell holding 'atomic' positions corresponding to the
   * Wannier center distribution
   ...
   * cell, one index per center in Wp
   * @param hr Wannier90 Hamiltonian as generated by readHr
   * @param tol tolerance level, interaction matrices with no entry above this
   * will be discarded
   */
  inline ll_hbonds(ll_cell cell, const fMat &Wp, const wi &I, const R_H &hr,
                   const double tol) noexcept
      : ll_hbonds(std::move(cell), Wp, I, hr, [tol](cMat &&inp) -> cMat {
          if (!std::any_of(inp.cbegin(), inp.cend(),
                           [tol](const auto &inp) -> bool {
                             return ll__::cmph(inp, tol);
                           }))
            return {};
   ...
   * @param Norb vector defining the number of Wannier centers or 'orbitals'
   * matched to each position
   * @param dat vector of index pairs, R vectors and interactions {i1,i2,R,H}
   */
  inline ll_hbonds(ll_cell cell, std::vector<size_t> Norb,
                   std::vector<ll__::i_i_R_H> dat) noexcept
      : ll_bonds<ll__::i_i_R_H>(std::move(cell), std::move(dat)),
        Norb_(std::move(Norb)) {}
...
public:
  ...
  inline size_t NHentries() const noexcept {
    return std::accumulate(
        this->cbegin(), this->cend(), size_t(0),
  ...
  inline size_t Norb(const std::string &s) const noexcept {
    return Norb(this->ind(s));
  }
  //! returns the number of orbitals for a range of id strings
  inline std::vector<size_t> Norb(const idv &id) const noexcept {
    return Norb(this->ind(id));
  }
  /** returns a 'similarized cell' for which the id strings are replaced by
   * strings of the form A{Norb}, e.g. 'A3' or 'A5'. All atoms with the same
  ...
  /** @name modification
   */
  //! function to filter bonds using a generic lambda on indices, start and end
  //! points, R and H
  size_t filter(
  ...
  //! function to filter bonds using a lambda on just {i1,i2} and start and end
  //! points {p1,p2}
  inline size_t filter(
      const std::function<bool(const size_t i1, const size_t i2, const fMat &p1,
                               const fMat &p2)> &eval) noexcept {
    return filter([&eval](const cMat &H, const fMat &B, const size_t i1,
                          const size_t i2, const fMat &p1, const fMat &p2,
                          const fCol &R) -> bool {
      return eval(i1, i2, B.prod(p1), B.prod(p2 + R));
    });
  }
  /** function to shift the spectrum, i.e. shift bandstructures produces from
   * bond interactions in energy. This shift must be applied to the
   * self-interactions, i.e. bonds of length 0.
  ...
  inline void shiftSpectrum(const std::complex<double> &sh) noexcept {
    for (const auto i : this->inds()) {
      const auto itr = std::lower_bound(this->vec_.begin(), this->vec_.end(),
                                        ll__::i_i{i, i});
  ...
  /** printing
   */
  /** function to write the wbh to a file. The Fermi energy can be included
   * since this way all the information to generate Hamiltonians/bandstructures
