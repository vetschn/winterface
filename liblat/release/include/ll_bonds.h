...
public:
  ...
  //! derived comparison
  inline bool operator>(const i_i &rhs) const noexcept { return rhs < *this; }
  //! derived comparison
  inline bool operator<=(const i_i &rhs) const noexcept {
...
public:
  ...
  inline std::ostream &print(std::ostream &os,
    ...
    auto j = ccBegin();
    size_t p1 = 0, p2 = s.find('\n');
    for (auto i = cbegin(), ie = cend(); i != ie;
         ++i, ++j, p1 = p2 + 1, p2 = s.find('\n', p1))
      os << s.substr(p1, p2 - p1) << " | " << std::fixed
         << std::setprecision(prec) << std::setw(prec + 4) << lm__::norm(*j)
         << ", " << *i << "\n";
    return os;
  }
  //! streaming operator
  friend inline std::ostream &operator<<(std::ostream &os,
                                         const b_I &inp) noexcept {
    inp.print(os);
    return os;
  }
};
...
public:
  ...
  inline ll_bonds<ITYPE>(ll_cell cell, std::vector<ITYPE> dat) noexcept
      : ll__::vec_cb<std::vector<ITYPE>, ITYPE>(std::move(dat)),
        cell_(std::move(cell)) {}
   ...
   */
  inline ll_bonds<ITYPE>(const ll_cell &cell, const fMat &R,
  ...
  inline ll_bonds<ITYPE>(const ll_cell &cell, const fMat &R, const double r,
                         const std::vector<size_t> &I1,
                         const std::vector<size_t> &I2) noexcept
      : ll_bonds<ITYPE>(
            cell, R, [r, &I1, &I2](const fMat &b, const i_i &j) -> bool {
              ...
              {
                const auto itr1 =
                    std::lower_bound(I1.cbegin(), I1.cend(), j.i1());
                const auto itr2 =
                    std::lower_bound(I2.cbegin(), I2.cend(), j.i2());
                if (itr1 != I1.cend() && *itr1 == j.i1() && itr2 != I2.cend() &&
                    *itr2 == j.i2())
                  return true;
              }
              ...
              {
                const auto itr1 =
                    std::lower_bound(I1.cbegin(), I1.cend(), j.i2());
                const auto itr2 =
                    std::lower_bound(I2.cbegin(), I2.cend(), j.i1());
                if (itr1 != I1.cend() && *itr1 == j.i2() && itr2 != I2.cend() &&
                    *itr2 == j.i1())
                  return true;
              }
  ...
  inline auto end(const size_t i) noexcept -> decltype(this->cend()) {
    return (i == NPOS__)
               ...
               : std::lower_bound(this->begin(), this->end(), i + 1,
                                  [](const auto &j, const auto i) -> bool {
                                    return j.i1() < i;
                                  });
  }
  ...
  //! const_iterator to beginning of the range of index i
  inline auto cbegin(const size_t i) const noexcept
      -> decltype(this->cbegin()) {
    return this->begin(i);
  }
  //! const_iterator past the end of the range of index i
  ...
  //! returns the maximum spacial tolerance for which bonds are distinguishable
  //! (in direct)
  ...
  inline fMat rmat() const noexcept {
    return std::accumulate(cbegin(), cend(), lm__::ones<fMat>(dim(), 1),
                           [](const fMat &s, const auto &i) -> fMat {
                             return s & ~lm__::nany(i.R());
                           });
  }
  ...
  inline size_t ind(const std::string &s) const noexcept {
    const auto itr =
        std::lower_bound(cell().id().cbegin(), cell().id().cend(), s);
    return itr != cell().id().cend() && s == *itr
               ? std::distance(cell().id().cbegin(), itr)
               : NPOS__;
  }
  ...
  /** returns the numbers of bonds starting or ending with an index of a range
   * of indices. Each occurrence is counted, i.e. both directions inverse bond
   * pairs and twice if the bond starts and ends at the same index. The number
   * is therefore always an even number.
   */
  inline std::vector<size_t> Nindex(std::vector<size_t> inds) const noexcept {
    std::transform(inds.cbegin(), inds.cend(), inds.begin(),
                   [this](const size_t i) -> size_t { return Nindex(i); });
    return inds;
  }
  /** returns an upper bound for the bond length.
   * e.g. all bonds fit within a sphere of this radius.
   * Does not considered starting an ending points of points, just the bond
   * vectors.
   */
  ...
  inline fMat boundary() const noexcept {
    ...
    // add due to tolerance level
    res += std::abs(lm__::mtol()) *
           lm__::nmax(
               lm__::abs(this->cell().B().prod(ll__::genNNmat(this->dim()))))
               .mat;
  ...
  /** returns the unique R vectors, i.e. the range of unit cells where bonds
   * exist but view in a different basis B
  ...
  /** returns the neighborhood for a bond index. i.e. all the bonds starting
   * from this index
   * @param ind starting index
   * @param Rcut cutoff radius for the bonds
   * @param sptol spacial tolerance used when sorting the bonds
   */
  inline b_I neighborhood(const size_t ind, const double Rcut,
                          const double sptol) const noexcept {
    assert(ind < cell().N());

    std::vector<i_i> I;
    I.reserve(this->Nindex(ind));
    fMat b(this->dim(), 0);
    b.reserve(I.capacity());
    ...
    for (auto i = this->cbegin(ind), ie = this->cend(ind); i != ie; ++i) {
      ...
      for (auto j = i->R().ccBegin(), je = i->R().ccEnd(); j != je; ++j) {
        ...
        if (lm__::norm(cb) < Rcut)
          b.push_back(cb), I.push_back({i->i1(), i->i2()});
      }
    }
  ...
  inline bool symmetric() const noexcept {
    for (const auto &j : *this) {
      ...
      const auto jtr =
          std::lower_bound(this->cbegin(), this->cend(), i_i(j.i2(), j.i1()));
  ...
  inline ll_cell getBondCenters() const noexcept {
    ...
    for (auto i = this->cbegin(), ie = this->cend(); i != ie; ++i) {
      ...
      for (auto r = i->ccBegin(),
                re = (i->i1() == i->i2() ? i->ccBegin() + i->N() / 2
                                         : i->ccEnd());
           r != re; ++r) {
        ...
        if (itr != bc.ccEnd() && *itr == cbc) {
          ...
          for (std::sregex_token_iterator i(bid[(size_t)itr].cbegin(),
                                            bid[(size_t)itr].cend(), r_, -1),
               e;
               i != e; ++i)
            buff.push_back(*i);
          ...
          buff.push_back(id1 < id2 ? '(' + id1 + ':' + id2 + ')'
                                   : '(' + id2 + ':' + id1 + ')');
   ...
   * @param prec floating point precision
   */
  inline void printToFile(const std::string &fileName, const size_t mode = 0,
                          const size_t prec = PPREC__) const {
    std::ofstream file;
    file.open(fileName);
    if (!file.good())
