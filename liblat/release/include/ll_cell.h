 ...
 * can be composited from other id strings such as Mo-001,S-002 ->
 * (Mo-001:S-002) which is the id strings given to the bond center between these
 * two atoms. In principle, this can go on indefinitely, e.g.
 * ((Mo-001:S-002):S-001) etc. Of particular importance is the changeBasis or
 * expandBasis function, which expresses the same lattice in terms of a
 * different periodicity as expressed in the basis B. The new and old
...
class ll_cell : public ll__::mat_cb<ll__::fMat, ll__::fArray> {
 ...
 public:
   ...
   * @param id id string
   */
  inline explicit ll_cell(fMat B, fMat Ap, aCv N,
  ...
  inline double sign() const noexcept {
    return empty() ? 1.0 : lm__::det(B()) < 0.0 ? -1.0 : 1.0;
  }
  ...
  inline static size_t getIndex(const std::string &id) noexcept {
    const size_t i = id.find_last_of(")-");
    return i != std::string::npos && id[i] == '-' ? std::stod(id.substr(i + 1))
                                                  : 0;
  }
  ...
  inline aT type(const std::string &s) const noexcept {
    ...
    return (itr == id().cend() || *itr != s)
               ? NPOS__
               : std::distance(id().cbegin(), itr);
  }
  ...
  /** returns a range of atomic types for whom the stripped id string is equal
   * to s
   * @param s id string
   */
   ...
   * @param t atomic type
   */
  inline aC Ntype(const aT t) const noexcept {
    return validType(t) ? T_[t + 1] - T_[t] : 0;
  }
  ...
  /** returns the least frequent type of a range of atomic types, i.e. of the
   * lowest atomic count
   * @param ts range of atomic types
   * @return atomic type
   */
  ...
  /** returns the most frequent type of a range of atomic types, i.e. of the
   * highest atomic count
   * @param ts range of atomic types
   * @return atomic type
   */
  ...
  inline std::vector<size_t> ind(const aT t) const noexcept {
    return validType(t) ? ll__::rg(T_[t], 1, T_[t + 1] - T_[t])
                        : std::vector<size_t>();
  }
   ...
   * The new basis must be valid for the same lattice but not of the same
   * symmetry group.
   * @param NB new basis
   */
  ...
  //! attempts to make the unit cell primitive, i.e. reduce the volume to the
  //! minimum.
  ...
  /** attempts to make the unit cell primitive, i.e. reduce the volume to the
   * minimum in a subspace. This will completely freeze one or more dimenions
   * and run the algorithm on the remaining unit cell of lower dimenions.
   ...
   * This will NOT cause a resorting of the resulting atomic positions.
   */
  inline ll_cell &shift(const fMat &sh) noexcept {
    assert(sh.L() == dim());
    return shift(sh.cAt(0));
  }
  /** attempts to automatically shift atomic positions.
   * This will try to maximize the 'leftness' of the unit cell, i.e. it will
   * attempt to place the largest gap between positions at the right of the
   * interval [0,1)
  ...
  /** this will 'diversify' a range of atomic types. Henceforth they will be
   * considered distinguished and the corresponding id strings will be
   * reindexed. Indexed ids always start from -001
   * @param ts range of atomic types
   */
  ll_cell &diversify(const aTv &ts) noexcept;
  /** this will 'diversify' all atomic types. Henceforth they will be considered
   * distinguished and the corresponding id strings will be reindexed. Indexed
   * ids always start from -001
   */
  inline ll_cell &diversify() noexcept { return diversify(types()); }
  /** this will 'collectivize' a range of atomic types. Henceforth they will be
   * considered of the same type. The new id string is that of the first in the
   * range
   * @param ts range of atomic types
   */
  ll_cell &collectivize(const aTv &ts) noexcept;
  /** this will 'collectivize' all atomic types. Henceforth they will be
   * considered of the same type. The new id string is that of the first in the
   * range
   */
  inline ll_cell &collectivize() noexcept { return collectivize(types()); }
  /** this will attempt to merge two unit cells into one. Same id strings
   * (including index) will be considered the same type.
   * @param inp the cell wot merge this one with
   */
  ll_cell &merge(const ll_cell &inp) noexcept;
  /** this will simplify composite ids since they can get very long in some
   * cases. They will be replaced with '(CT)' and indexed afterwards.
   */
  ...
  /** returns atomic positions for a range of atomic types in cartesian
   * coordinates
   * @param ts range of atomic types
   */
  inline fMat getcAp(const aTv &ts) const noexcept {
    return B().prod(getAp(ts));
  }
   ...
   * Bonds across boundaries to neighbor cells can be included or excluded with
   * an appropriate next neighbor matrix.
   * @param NN next neighbor matrix
   * @param keep lambda to determine which bonds to include
   */
  ll_bonds<i_i_R> getBonds(const fMat &NN,
                           const std::function<bool(const fMat &, const i_i &)>
                               &keep) const noexcept;
  /** returns bonds between atomic positions of a certain type with a certain
   * type. Bonds across boundaries to neighbor cells can be included or excluded
   * with an appropriate next neighbor matrix.
   * @param T1 range of atomic types (start points)
   * @param T2 range of atomic types (ending points)
   * @param f tolerance factor, f<0: upper limit is |f| times shortest bond
   * length f>0: upper limit is f
   * @param NN next neighbor matrix
   */
  ll_bonds<i_i_R> getBonds(const aTv &T1, const aTv &T2, const double f,
  ...
  /** @name comparison
   */
  /** returns a permutation matrix if the other cell is the same upto a
   * permutation of the unit cell. Returns empty otherwise.
  ...
  /** returns a shift vector if the other cell is the same upto a permutation or
   * a shift. Returns empty otherwise.
   * @param inp other cell to compare to
  ...
  inline bool operator==(const ll_cell &rhs) const noexcept {
    return B() == rhs.B() && Ap() == rhs.Ap() && T_ == rhs.T_ &&
           id() == rhs.id();
  }
  //! compares all containers directly
  inline bool operator!=(const ll_cell &rhs) const noexcept {
   ...
   */
  std::string print(const bool direct = true,
                    const size_t prec = PPREC__) const noexcept;
 ...
 protected:
   ...
   */
  inline static std::string appendIndex_(const std::string &id,
                                         const size_t i) noexcept {
    std::stringstream sstr;
    sstr << id << '-' << std::setfill('0') << std::setw(NLZ__ + 1) << i;
    return sstr.str();
  }
  //! detect file type
