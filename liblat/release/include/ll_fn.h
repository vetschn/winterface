 ...
 * @param tol tolerance level as when a floating point number can be considered
 * an integer
 */
fMat findBasisExpansion(const fMat &B, const fMat &V, const double lim = 1000.0,
 ...
 * an integer
 */
inline fMat findBasis(const fMat &B, const fMat &V, const double lim = 1000.0,
                      const double tol = 1e-2) {
  return B.prod(findBasisExpansion(B, V, lim, tol));
}
/** function to 'orthogonalize' a basis from a restriction vector. This function
 * will try to make basis vectors deemed restricted orthogonal to the others
 * @param B basis
 ...
 * @param Nps vector holding the number of points along each path between two
 * points in P
 */
p_p genPath(const fMat &P, std::vector<size_t> Nps);
/** generate points along a path including position information. The number of
 * points for each segment is determined automatically by the relative distance
 * of the points in P.
 * @param P points matrix
 ...
 */
inline fMat genNNmat(const size_t d = DIM__) noexcept {
  return genNNmat(rv(d, false));
}
 ...
 */
double dist(const fMat &B, const fArray &p1, const fArray &p2,
            fMat NN) noexcept;
/** function to find the distance of two points in periodic space, tolerant
 * mode. This version will keep increasing the result from the minimum until a
 * large enough gap is found. Useful for slightly warped lattices.
 * @param B basis
 * @param p1 the first point in direct coordinates
 * @param p2 the first point in direct coordinates
 ...
 * @param NN next neighbor matrix
 */
double dist(const fMat &B, const fArray &p1, const fArray &p2, const double f,
            fMat NN) noexcept;
/** function to find the distance of two points in periodic space
 * @param B basis
 * @param p1 the first point in direct coordinates
 ...
 * 		indicating which image is the closest
 */
d_b distb(const fMat &B, const fArray &p1, const fArray &p2, const double f,
          fMat NN) noexcept;
 ...
 * @param NN next neighbor matrix
 */
fMat genDmat(const fMat &B, const fMat &P1, const fMat &P2,
             const fMat &NN) noexcept;
/** function to return a distance matrix of points with themselves.
 * This version will keep increasing the distance from the minimum until a large
 * enough gap is found. Useful for slightly warped lattices.
 * @param B basis
 * @param P positions in direct coordinates
 * @param f tolerance factor, f<0: upper limit is |f| times shortest bond length
 *                            f>0: upper limit is f
 * @param NN next neighbor matrix
 */
fMat genDmat(const fMat &B, const fMat &P, const double f,
             const fMat &NN) noexcept;
/** function to return a distance matrix of one set of points with another.
 * This version will keep increasing the distance from the minimum until a large
 * enough gap is found. Useful for slightly warped lattices.
 ...
 * @param NN next neighbor matrix
 */
fMat genDmat(const fMat &B, const fMat &P1, const fMat &P2, const double f,
             const fMat &NN) noexcept;
...
/** function to generate a grouping of points into clusters using the dbscan
 * algorithm.
 ...
 * @param R R vectors
 * @param strict switch to allow only exact matched(true) or also approximate
 * matches(false)
 */
template <class MT, class WT>
R_H<MT> genHam(const fMat &B, const fMat &Ap, const idv &id, const WT &W,
               fMat R, const bool strict) noexcept;
/** generate a Hamiltonian in Wannier representation
 * @param B basis
 * @param Ap atomic postions
 * @param id vector of id strings
 ...
 */
template <class MT, class WT>
R_H<MT> genHam(const fMat &B, const fMat &Ap, const idv &id, const rv &r,
 ...
 */
template <class MT, class WT>
R_H<MT> inline genHam(const ll_cell &cell, const WT &W, fMat R,
                      const bool strict) noexcept {
  return genHam<MT>(cell.B(), cell.getcAp(), cell.id(cell.type()), W, R,
                    strict);
}
/** generate a Hamiltonian in Wannier representation
 * @param cell unit cell
 ...
 */
template <class MT, class WT>
R_H<MT> inline genHam(const ll_cell &cell, const rv &r, const WT &W,
                      const bool strict) noexcept {
  return genHam<MT>(cell.B(), cell.getcAp(), cell.id(cell.type()), r, W,
                    strict);
}
 ...
 */
template <class MT, class WT>
R_H<MT> inline genHam(const WT &W, fMat R) noexcept {
 ...
 * @param NN next neighbor matrix
 */
wi matchCenters(const fMat &B, const fMat &Ap, const fMat &Wp,
                const fMat &NN) noexcept;
/** perform 'wannier matching' by matching Wannier centers to 'atomic' positions
 * @param cell unit cell
 * @param Wp Wannier centers
 * @param NN next neighbor matrix
 */
inline wi matchCenters(const ll_cell &cell, const fMat &Wp,
                       const fMat &NN) noexcept {
  return matchCenters(cell.B(), cell.Ap(), Wp, NN);
}
/** function to 'balance' out a Wannier matching. This can be useful for
 * suboptimal Wannierizations where some Wannier centers can be a bit further
 * away than usual. They may then get matched to the 'wrong' atom. This function
 * attempts to balance things out by shifting the least obvious matches such
 * that the same atomic types all have the same number of Wannier centers
 * matched to them despite the spacial warping of the initial distribution.
 * @param I 'naive' Wannier matching
 * @param cell unit cell
 * @param Wp Wannier centers
 * @param NN next neighbor matrix
 */
void balance(wi &I, const ll_cell &cell, const fMat &Wp,
             const fMat &NN) noexcept;
/** function to 'clusterize' Wannier centers. This attempts to find clusters of
 * Wannier centers and matches them to each other.
 * @param B basis
 * @param Wp Wannier centers
 * @param eps small parameter, paramter for dbscan
 * @param minpts minimum number of points to count as a cluster, parameter for
 * dbscan
 * @param NN next neighbor matrix
 */
wi clusterize(const fMat &B, const fMat &Wp, const double eps,
              const size_t minpts, const fMat &NN) noexcept;
/** function to find 'atomic' positions for 'clusterized' Wannier centers. This
 * uses the 'center of mass' as the 'atomic' positition with the Wannier spreads
 * as weights.
 * @param B basis
 * @param Wp Wannier centers
 ...
 */
fMat genCenters(const fMat &B, const fMat &Wp, const fMat &s,
                const wi &I) noexcept;
...
//! function to find the largest entries of real and imaginary parts in a
//! Wannier Hamiltonian
 ...
 */
template <class MT>
fMat calcBS(const R_H<MT> &hr, const fMat &k,
 ...
 */
template <class MT>
inline ll_mesh<> calcBS(const R_H<MT> &hr, const ll_mesh<> &k,
                        const size_t Nthreads = NTHREADS__) noexcept {
  return ll_mesh<>(calcBS(hr, k.base_, Nthreads), k.maj(), k.D());
}
/** function to compute the bandstructure on a trace for a super cell using a
 * folding approach.
 * @param hr Hamiltonian in Wannier representation
 * @param k kpoints
 ...
 */
template <class MT>
fMat calcFoldedBS(const R_H<MT> &hr, const fMat &k, const fMat &B,
                  const fMat &Bp, const size_t Nthreads = NTHREADS__) noexcept;
/** function to compute the bandstructure on a mesh for a super cell using a
 * folding approach.
 * @param hr Hamiltonian in Wannier representation
 * @param k mesh of kpoints
 * @param B basis of the super cell
 ...
 */
template <class MT>
inline ll_mesh<> calcFoldedBS(const R_H<MT> &hr, const ll_mesh<> &k,
                              const fMat &B, const fMat &Bp,
                              const size_t Nthreads = NTHREADS__) noexcept {
...
/** function to compute the bandstructure including gradient and curvature on a
 * trace.
 * @param hr Hamiltonian in Wannier representation
 * @param k kpoints
 ...
 */
template <class MT>
inline egc<ll_mesh<>> calcBS_gc(const R_H<MT> &hr, const ll_mesh<> &k,
                                const fMat &B,
                                const size_t Nthreads = NTHREADS__) noexcept {
  ...
  return {ll_mesh<>(std::move(buff.E), k.maj(), k.D()),
          ll_mesh<>(std::move(buff.G.base_), cat(0, k.maj() + 1),
                    cat(k.M(), k.D())),
          ll_mesh<>(std::move(buff.C.base_), cat(0, 1, k.maj() + 2),
                    cat(k.M(), k.M(), k.D()))};
}
/** function to compute the bandstructure including gradient and curvature on a
 * trace in a super cell using a folding approach.
 * @param hr Hamiltonian in Wannier representation
 * @param k kpoints
 ...
 */
template <class MT>
egc<fMat> calcFoldedBS_gc(const R_H<MT> &hr, const fMat &k, const fMat &B,
                          const fMat &Bp,
                          const size_t Nthreads = NTHREADS__) noexcept;
/** function to compute the bandstructure including gradient and curvature on a
 * mesh in a super cell using a folding approach.
 * @param hr Hamiltonian in Wannier representation
 * @param k mesh of kpoints
 * @param B basis of the super cell (real space)
 * @param Bp basis of the primtive cell (real space)
 ...
 * @param tol tolerance level as to which matches should be included. Matches
 * with interaction matrices with no entry exceeding this are discarded.
