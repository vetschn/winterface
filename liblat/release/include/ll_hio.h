...
public:
  ...
  inline explicit ll_writer(lm__::fMat R) noexcept
      : R_(std::move(R)), r_(R_.ccBegin()), nnz_(R_.N(), 0), n_(nnz_.begin()) {
...
public:
  ...
  inline std::string c_descr() const noexcept {
    return lm__::T(this->c_R()).print(0);
  }
...
private:
   ...
   */
  //! the number of bytes for each Hamiltonian entry
  inline size_t bytes_() const noexcept {
    return 2 * sizeof(size_t) + sizeof(ll__::hel);
  }
  /** write data blob function
   * @param inp vector of Hamiltonian entries
   */
...
//! entries
template <>
inline void ll_writerBIN<uint16_t, float>::insert_(const hdat &inp) {
  for (const auto &i : inp) {
...
//! insert specialization for 32bit indices and single precision Hamiltonian
//! entries
template <>
inline void ll_writerBIN<uint32_t, float>::insert_(const hdat &inp) {
  for (const auto &i : inp) {
...
ll_writerBIN<uint16_t, std::complex<float>>::insert_(const hdat &inp) {
  for (const auto &i : inp) {
    const uint16_t nm[] = {uint16_t(i.m), uint16_t(i.n)};
    file_.write((char *)&nm, 2 * sizeof(uint16_t));
    const std::complex<float> h = {float(std::real(i.h)),
                                   float(std::imag(i.h))};
...
//! insert specialization for 16bit indices and double precision Hamiltonian
//! entries
template <>
inline void ll_writerBIN<uint16_t, double>::insert_(const hdat &inp) {
  for (const auto &i : inp) {
    const uint16_t nm[] = {uint16_t(i.m), uint16_t(i.n)};
    file_.write((char *)&nm, 2 * sizeof(uint16_t));
...
//! insert specialization for 32bit indices and double precision Hamiltonian
//! entries
template <>
inline void ll_writerBIN<uint32_t, double>::insert_(const hdat &inp) {
  for (const auto &i : inp) {
    const uint32_t nm[] = {uint32_t(i.m), uint32_t(i.n)};
    file_.write((char *)&nm, 2 * sizeof(uint32_t));
    const double h = std::real(i.h);
...
//! header specializations for 16bit indices and single precision Hamiltonian
//! entries
template <>
inline const char *ll_writerBIN<uint16_t, float>::header_() noexcept {
  return "hrssr";
}
//! header specializations for 32bit indices and single precision Hamiltonian
//! entries
template <>
inline const char *ll_writerBIN<uint32_t, float>::header_() noexcept {
  return "hrlsr";
}
//! header specializations for 16bit indices and double precision Hamiltonian
//! entries
template <>
inline const char *ll_writerBIN<uint16_t, double>::header_() noexcept {
  return "hrsdr";
}
//! header specializations for 32bit indices and double precision Hamiltonian
//! entries
template <>
inline const char *ll_writerBIN<uint32_t, double>::header_() noexcept {
  return "hrldr";
}
//! header specializations for 16bit indices and single precision complex
//! Hamiltonian entries
template <>
inline const char *
ll_writerBIN<uint16_t, std::complex<float>>::header_() noexcept {
  return "hrssc";
}
//! header specializations for 32bit indices and single precision complex
//! Hamiltonian entries
template <>
inline const char *
ll_writerBIN<uint32_t, std::complex<float>>::header_() noexcept {
  return "hrlsc";
}
//! header specializations for 16bit indices and double precision complex
//! Hamiltonian entries
template <>
inline const char *
ll_writerBIN<uint16_t, std::complex<double>>::header_() noexcept {
  return "hrsdc";
}
//! header specializations for 32bit indices and double precision complex
//! Hamiltonian entries
template <>
inline const char *
ll_writerBIN<uint32_t, std::complex<double>>::header_() noexcept {
  return "hrldc";
}
 ...
 * This function attempts to find the R vectors for which Hamiltonian entries
 * exist by scanning over a set of adjacent unit cells.
 ...
 * @param approx switch to enable approximate interactions (true) or enforce
 * exact matching (false)
 */
template <class WT>
lm__::fMat getConnectedGrid(const WT &W, const lm__::fMat &B,
                            const lm__::fMat &Ap, const aTv &T, const rv &r,
...
inline lm__::fMat getConnectedGrid(const ll_hbonds &W, const rv &r,
  ...
  return getConnectedGrid(W, W.cell().B(), W.cell().Ap(), W.cell().type(), r,
                          IR, approx);
}
...
void hctor(const WT &W, const fMat &B, const fMat &Ap, const aTv &T,
           ...
           const size_t Nthreads = 0, const size_t verbosity = 0,
 ...
 * @param os stream to print into
 */
inline void hctor(const ll_hbonds &W, ll_writer &writer, const double IR,
                  const bool approx, const size_t Nthreads = 0,
                  const size_t verbosity = 0, std::ostream &os = std::cout) {
  hctor(W, W.cell().B(), W.cell().getcAp(), W.cell().type(), writer, IR, approx,
        Nthreads, verbosity, os);
}
 ...
 * @param os stream to print into
 */
void adaptWBH(ll_hbondss &W, const ll_hbondss_input &inp, const fMat &Ap,
