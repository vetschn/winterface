    ...
    inline explicit printHelp_(const ll_wf_input &p, std::ostream &os)
      ...
      printHelpTuple_(
          os, std::make_tuple(
                  ...
                  HELP__("weig", p.weig, "name of the wannier90 *.weig file"),
                  HELP__("outcar", p.outcar, "name of the VASP OUTCAR file"),
                  HELP__("Ef", p.Ef, "manual specification of the Fermi energy")

                      ,
                  TOPIC__("OMEN: BASIS EXPANSION"),
                  HELP__("itol", p.itol,
                         "cutoff tolerance used in fraction detection, higher "
                         "values may\n"
                         "help with automatic basis detection"),
                  HELP__("xyz", p.xyz, DIM__, DIM__,
                         "matrix to specify where x,y and z directions should "
                         "point to, in cartesian.\n"
                         "this is used to find the expansion from the "
                         "primitive cell to the smallest\n"
                         "orthorhombic cell, rows must be perpendicular"),
                  HELP__("S", p.S, DIM__, DIM__,
                         "stress tensor to be applied to primitive basis "
                         "before anything else\n"
                         "this is useful in case no exact orthorhombic "
                         "supercell exists\n"
                         "beware! there are no enforced boundaries, do not use "
                         "this for anything else!\n"
                         "as a warning frobenius(eye-S) is printed, if this is "
                         "not small you are doing\n"
                         "something fishy"),
                  HELP__("C", p.C, DIM__, DIM__,
                         "expansion coefficient matrix overwrite for expansion "
                         "from primitive to\n"
                         "minimal orthorhombic, expansion for each new vector "
                         "in terms of the old\n"
                         "ones must be given in the rows"),
                  HELP__("l", p.l, DIM__,
                         "expansion coefficients overwrite for minimal "
                         "orthorhombic basis to NN only"),
                  HELP__("vac", p.vac,
                         "vacuum modification length, replaces large value "
                         "used in DFT\n"
                         "if this value is lower than a bond length, the bond "
                         "length is used"),
                  HELP__("filter_wbh", p.filter_wbh,
                         "switch to turn on/off filtering the bonds in the wbh "
                         "in each direction\n"
                  ...
                  HELP__("IR", p.IR,
                         "interaction radius cutoff, any bonds of length above "
                         "this value\n"
                         "will be discarded when scaling hamiltonians. If this "
                         "is not set manually\n"
                  ...
                  HELP__("device_length,", p.device_length,
                         "total device length [ANGSTROM] to use in the OMEN "
                         "stump cmd file"),
                  HELP__("force_check", p.force_check,
                         "switch to force the consistency check of hamiltonian "
                         "matrices\n"
  ...
  struct printHelp_ : public virtual aux_parser::printHelp_ {
    ...
    inline explicit printHelp_(const ll_ph_input &p, std::ostream &os)
      ...
      printHelpTuple_(os, std::make_tuple(
                              ...
                              HELP__("myfile", p.myfile, "myfile n stuff")));
    }
  };
};
...
inline fMat rToR(const rv &r) noexcept {
  ...
  fMat res(DIM__, 0);
  res.reserve(size_t(std::pow(DIM__, sum(bnds))));
...
public:
   ...
   */
  inline explicit writer(lm__::fMat R, const size_t Nw,
                         std::string prefix = "./")
...
protected:
  ...
  inline size_t bytes_() const noexcept {
    return 2 * sizeof(double) + sizeof(ll__::hel);
  }
  ...
  inline void insert_(const hdat &inp) {
    for (const auto &i : inp) {
      double buff[] = {double(i.m), double(i.n), std::real(i.h),
                       std::imag(i.h)};
 ...
 * @param os stream to print into
 */
void hctor(const ll_wf_input &inp, const fMat &LM, const idv &id, const fMat &L,
           std::ostream &os);
