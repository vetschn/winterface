  ...
  try {
    ...
    switch (fnvHash(inp.mode.front().c_str())) {
      ...
      case "wannier"_h:
        ...
        I.reserve(w.N());
        for (size_t i = 0; i != w.N(); ++i) I.push_back({i});
        break;
      case "atomic"_h: {
      ...
      case "composite"_h: {
        if (inp.mode.size() == 1)
          throw(
              std::invalid_argument("composite mode: "
    ...
    if (inp.balance_matches) balance(I, cell, w.Wp() % 1.0, NN);
...
ll_hbonds::ll_hbonds(ll_cell cell, const fMat& Wp, const wi& I, const R_H& hr,
  ...
  for (const auto& i : I) Norb_.push_back(i.size());
  ...
  for (auto p1 = this->cell().ccBegin(), pe = this->cell().ccEnd(); p1 != pe;
    ...
    for (auto p2 = this->cell().ccBegin(); p2 != pe; ++p2) {
      ...
      for (auto r = hr.ccBegin(), re = hr.ccEnd(); r != re; ++r) {
        ...
        if (!H.empty()) vec_.back().push_back(*r, std::move(H));
      }
...
// searching
ll_hbonds::am_ ll_hbonds::getApproximateInteraction(
    ...
    case 2: {
      ...
      for (const auto& i : *this)
        os << std::setw(11) << std::left << (ll__::i_i)i << std::setw(21)
           ...
           << (" NR = " + std::to_string(i.R().N())) << "[" << std::setw(lwidth)
           << std::left << nmin(i.R()).mat.T().print(0) << "; "
           << std::setw(uwidth) << std::left << nmax(i.R()).mat.T().print(0)
           << "]\n";
    } break;
  }
