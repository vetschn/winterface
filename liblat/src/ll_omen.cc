...
ll__::omen::prepper::prepper(const ll_wf_input& inp, std::ostream& os) {
  ...
  // find smallest orthorhombic cell
  {
    ...
    if (!inp.C.empty()) {
      ...
      if (msize(inp.C) != msize(W.cell().B()) || inp.C != round(inp.C))
        throw(std::invalid_argument(
            "C must be a square matrix of integers of size(" +
      ...
      if (msize(inp.xyz) != msize(W.cell().B()) || !inp.xyz.ob())
        throw(std::invalid_argument(
            "xyz must be a square orthogonal matrix of size(" +
            std::to_string(DIM__) + "," + std::to_string(DIM__) + ")"));
    ...
    if (inp.C.empty() && inp.S.empty()) {
      ...
      Bhat.reserve(B.N());
      auto s = P.cdbegin();
      for (auto i = B.ccBegin(), e = B.ccEnd(), j = inp.xyz.ccBegin(); i != e;
           ++i, ++j, ++s)
    ...
    if (inp.verbosity & PRINTBIT__) {
      ...
      os << "strain applied, frobenius(1-S): " << (sn < .05 ? GREEN__ : RED__)
         << sn << RESET__ << "\n";
    }
  ...
  if (inp.verbosity & PRINTBIT__) {
    ...
    os << "expansion to nn only is (" << (fcnt ? RED__ : GREEN__) << fcnt
       << RESET__ << "):\n"
       << T(NNE).print(0, 1) << "\n"
       << "\nresulting in " << BLUE__ << ORcell.N() << RESET__
       << " positions and " << BLUE__ << Ntot << RESET__ " orbitals total\n"
...
void ll__::omen::writeInput(const ll_wf_input& inp, std::ostream& os) {
  ...
  {
    std::ofstream file;
    file.open(inp.prefix + "Ef.dat");
    if (!file.good())
      throw(std::invalid_argument("open file \'" + inp.prefix + "Ef.dat" +
                                  "\' failed"));
    file << Ef;
  }
  ...
  size_t Nnn;
  {
    const auto b = P.W.cell()
                       .getSubCell(P.W.cell().fundamentalTypes())
  ...
  for (const auto i : inds(P.r))
    B_.cAt(i) *=
        (max(Ap_.rAt(i)) - min(Ap_.rAt(i)) + (inp.vac >= bl ? inp.vac : bl)) /
        norm(B_.cAt(i));
  ...
  // write material file
  printOmf(OMF__, E, P.W.Norb(uid_), P.W.cell().mass(uid_));
  if (inp.verbosity & PRINTBIT__)
...
void ll__::omen::callHamiltonianConstructor(double* const Layer_Matrix,
  ...
  try {
    ...
    // check timestamps
    {
      ...
      if (!ierr && st_h4.st_mtime > st_olf.st_mtime &&
          st_h4.st_mtime > st_omf.st_mtime)
        return;
    }
