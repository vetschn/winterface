...
void test_cell_ctor::test_B_Ap_() {
  ...
  // try inserting unordered data, with id
  {
    ...
    const fMat Ap_s({0.0,  0.0,  0.0,  0.0,  0.5,  0.5,  0.5,  0.0,
                     0.5,  0.5,  0.5,  0.0,  0.25, 0.25, 0.25, 0.25,
                     0.75, 0.75, 0.75, 0.25, 0.75, 0.75, 0.75, 0.25},
                    3, 8);
...
void test_cell_ctor::test_file() {
  ...
  // check POSCAR1
  {
    ...
    const fMat B({0.0, 0.5, 0.5, 0.5, 0.0, 0.5, 0.5, 0.5, 0.0}, 3, 3);

    const fMat Ap({0.0, 0.0, 0.0, 0.25, 0.25, 0.25}, 3, 2);
  ...
  // check POSCAR3
  {
    ...
    const fMat B({1.7, 0.0, 2.94449, 0.0, 29.893856, 0.0, -1.7, 0.0, 2.94449},
                 3, 3);
    ...
    const fMat Ap({0.0000000000000004, 0.0561988389855091, 0.0000000000000000,
                   ...
                   0.3333333333333338, 0.1123976779710184, 0.3333333333333335},
                  3, 6);
  ...
  // check wannier90 wout
  {
    ...
    const fMat B({3.400000, 0.000000, 0.000000, 0.000000, 29.893856, 0.000000,
                  -1.700000, 0.000000, 2.944486},
                 3, 3);
    const fMat Ap({0.00000, 1.68000, 0.00000, -0.00000, 8.21385, 1.96299,
                   -0.00000, 0.00000, 1.96299, 0.00000, 9.89385, 0.00000,
                   0.00000, 6.53387, 0.00000, -0.00000, 3.36001, 1.96299},
                  3, 6);
    const idv id = {"Mo", "S"};
    const aCv N = {2, 4};

    const ll_cell ref(std::move(B), B.leftDivide(Ap) % 1.0, std::move(N),
                      std::move(id));
  ...
  // check lattice_dat
  {
    ...
    printOlf(fileName, tCell1.B(), tCell1.getcAp(), tCell1.id(tCell1.type()), 0,
             .0);
