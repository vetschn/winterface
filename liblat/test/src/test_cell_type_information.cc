...
void test_cell_type_information::test_type() {
  ...
  CPPUNIT_ASSERT(std::all_of(T.cbegin(), T.cend(), [&tCell](const aT t) {
    return tCell.validType(t);
  }));
...
void test_cell_type_information::test_leastFreqType_mostFreqType() {
  ...
  CPPUNIT_ASSERT_EQUAL(*std::min_element(N.begin(), N.end()),
                       tCell.Ntype(tCell.leastFreqType()));
  CPPUNIT_ASSERT_EQUAL(*std::max_element(N.begin(), N.end()),
                       tCell.Ntype(tCell.mostFreqType()));
}
...
void test_cell_type_information::test_ind() {
  ...
  for (auto t : tCell.types()) {
    ...
    CPPUNIT_ASSERT(std::all_of(I.cbegin(), I.cend(), [&tCell](const size_t i) {
      return i < tCell.N();
    }));
...
CppUnit::Test* test_cell_type_information::suite() {
  ...
  suite->addTest(new CppUnit::TestCaller<test_cell_type_information>(
      "test_leastFreqType_mostFreqType",
      &test_cell_type_information::test_leastFreqType_mostFreqType));
  suite->addTest(new CppUnit::TestCaller<test_cell_type_information>(
