...
void test_fn_linear_path_generation::test_genPath() {
  ...
  // explicit number of points version
  {
    ...
    {
      CPPUNIT_ASSERT_THROW(genPath(fMat(3, 5), {3, 4, 3}),
                           std::invalid_argument);
      CPPUNIT_ASSERT_THROW(genPath(fMat(3, 5), {3, 4, 3, 1}),
                           std::invalid_argument);
    }
    ...
    // check randomly generated P, Nps
    {
      for (size_t i = 0; i != smax; ++i) {
        ...
        // check that points are on a straight line
        {
          size_t sind = 0;
          for (size_t j = 0; j != Nps.size(); ++j) {
            const auto v = path.cAt(sind + 1) - path.cAt(sind);
  ...
  // total number of points, equal spacing version
  {
    ...
    // check randomly generated P, Np
    {
      for (size_t i = 0; i != smax; ++i) {
        ...
        const size_t Np =
            rand<fMat>(1, 1, double(10 * s) - .499, double(30 * s) + 0.499)[0];
        ...
        for (size_t j = 0; j != P.N(); ++j) {
          inds[j] =
              std::find(path.ccBegin() + cind, path.ccEnd(), P.cAt(j)).i();
        ...
        for (size_t j = 0; j != Nps.size(); ++j)
          CPPUNIT_ASSERT(std::abs(norm(P.cAt(j + 1) - P.cAt(j)) / l -
                                  double(Nps[j]) / double(Np)) < 1e-1);
