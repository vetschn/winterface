...
void test_hbonds_all::test_ctor_exceptions() {
  ...
  // bad input files
  {
    ...
    CPPUNIT_ASSERT_THROW(ll_hbonds(inp, std::cout), std::invalid_argument);
...
void test_hbonds_all::test_all() {
  ...
  const ll_hbonds W(inp, std::cout);
  {
    ...
    for (auto i = b.ccBegin(), j = b.ccEnd() - 1, ie = b.ccEnd(); i != ie;
         ++i, --j)
      CPPUNIT_ASSERT(*i == -(*j));
  ...
  // compare full vs cutoff
  {
    ...
    // check all bonds/interactions from wbh_ are also found in wbh
    {
      auto j = b.cbegin();
      for (auto i = b.ccBegin(), ie = b.ccEnd(); i != ie; ++i, ++j)
        CPPUNIT_ASSERT(W_.getInteraction(*j, *i) == W.getInteraction(*j, *i));
    }
    reset_mtol();
  }
  ...
  // try writing to disk, then reading back in and compare and check exceptions
  {
    ...
    // bad file exceptions
    CPPUNIT_ASSERT_THROW(ll_hbonds(prefix + "hyugsa.wad"),
                         std::invalid_argument);
    CPPUNIT_ASSERT_THROW(W.writeToFile(prefix + "/not_exist/BH.wad"),
                         std::invalid_argument);

    // bad header exception
    CPPUNIT_ASSERT_THROW(ll_hbonds(path + "bad_header.wad"),
                         std::runtime_error);
  }
}
