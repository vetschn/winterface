...
void test_io::test_writePOSCAR() {
  ...
  {
    // B not empty
    CPPUNIT_ASSERT_THROW(printPOSCAR("bla", fMat(), fMat(), aTv()),
                         std::invalid_argument);

    // B not square
    CPPUNIT_ASSERT_THROW(printPOSCAR("bla", fMat(3, 2), fMat(3, 4), aTv()),
                         std::invalid_argument);

    // B.M()!=Ap.M()
    CPPUNIT_ASSERT_THROW(printPOSCAR("bla", fMat(3), fMat(4, 5), aTv()),
                         std::invalid_argument);

    // !T.empty() && T.size()!=Ap.N()
    CPPUNIT_ASSERT_THROW(
        printPOSCAR("bla", fMat(3), fMat(3, 4), {0, 1}, {}, 1.0, true),
        std::invalid_argument);

    // legal input no throw
    CPPUNIT_ASSERT_NO_THROW(printPOSCAR(prefix + "POSCAR_legal", fMat(3),
                                        fMat(3, 4), {0, 0, 1, 1}, {"S", "Mo"},
                                        1.0, true));
  }
  ...
  // random cell, direct
  {
    ...
    printPOSCAR(prefix + "POSCAR1", tCell1, true, false, 16,
                "random_POSCAR_test_direct");
  ...
  {
    const auto tCell1 = genRandom(D);
    printPOSCAR(prefix + "POSCAR2", tCell1, false, false, 16,
                "random_POSCAR_test_cart");
  ...
  {
    const auto tCell1 = genRandom(D);
    printPOSCAR(prefix + "POSCAR3", tCell1, true, false, 16,
                "random_POSCAR_test_cell");
...
void test_io::test_printOmf() {
  // check exceptions
  {
    CPPUNIT_ASSERT_THROW(printOmf("bla", {-1.0, 1.0}, {}, {1.3}),
                         std::invalid_argument);
    CPPUNIT_ASSERT_THROW(printOmf("bla", {-1.0, 1.0}, {2, 3}, {1.3}),
                         std::invalid_argument);
  }
...
void test_io::test_printOlf() {
  ...
  {
    CPPUNIT_ASSERT_THROW(
        printOlf("bla", {}, tCell.B().prod(tCell.Ap()), id, 6, 1.0),
        std::invalid_argument);
    CPPUNIT_ASSERT_THROW(printOlf("bla", zeros<fMat>(3, 2),
                                  tCell.B().prod(tCell.Ap()), id, 6, 1.0),
                         std::invalid_argument);
    CPPUNIT_ASSERT_THROW(
        printOlf("bla", zeros<fMat>(4), tCell.B().prod(tCell.Ap()), id, 6, 1.0),
        std::invalid_argument);
    CPPUNIT_ASSERT_THROW(
        printOlf("bla", tCell.B(), tCell.B().prod(tCell.Ap()), {"A1"}, 6, 1.0),
        std::invalid_argument);
  }
  ...
  // write test file
  {
    ...
    printOlf(fileName, tCell.B(), tCell.B().prod(tCell.Ap()), id, nn, bl);
  }
}
...
void test_io::test_readOmf() {
  ...
  CPPUNIT_ASSERT(std::vector<double>({1.1, 2.1, 3.1, 3.4, 5.4, 1.0}) ==
                 inp.mass);
}
...
void test_io::test_readB() {
  ...
  // check data is read correctly
  {
    // without P
    {
      ...
      const fMat ck({3.4, 0.0, 0.0, 0.0, 29.893856, 0.0, -1.7, 0.0, 2.944486},
                    3, 3);
      CPPUNIT_ASSERT(B == ck);
    }
  }
...
void test_io::test_readAp() {
  ...
  {
    // check bad file
    CPPUNIT_ASSERT_THROW(readAp(opath + "Layer_Matrix.dat"),
                         std::runtime_error);
    ...
    CPPUNIT_ASSERT_THROW(readAp(wpath + "wannier90.wout", true, {3, 4, 1, 0}),
                         std::invalid_argument);

    // check abl duplicate entries
    CPPUNIT_ASSERT_THROW(readAp(wpath + "wannier90.wout", true, {0, 1, 1, 2}),
                         std::invalid_argument);

    // check abl illegal entries
    CPPUNIT_ASSERT_THROW(
        readAp(wpath + "wannier90.wout", true, {0, 1, 2, 172645}),
        std::invalid_argument);
  ...
  // direct, with abl
  {
    ...
    const fMat ck({0.0, 0.05620, 0.0, 0.33333, 0.0, 0.66667, 0.0, 0.21857, 0.0,
                   0.33333, 0.11240, 0.66667},
                  3, 4);
  ...
  // direct, with abl, one type missing
  {
    ...
    const fMat ck({0.33333, 0.0, 0.66667, 0.0, 0.33097, 0.0, 0.0, 0.21857, 0.0,
                   0.33333, 0.11240, 0.66667},
                  3, 4);
  ...
  // cartesian
  {
    ...
    const fMat ck({0.0, 1.68, 0.0, 0.0, 8.21385, 1.96299, 0.0, 0.0, 1.96299,
                   0.0, 9.89385, 0.0, 0.0, 6.53387, 0.0, 0.0, 3.36001, 1.96299},
                  3, 6);
    const idv ckid = {"Mo", "Mo", "S", "S", "S", "S"};
  ...
  // cartesian, with abl;
  {
    ...
    const fMat ck({0.0, 1.68, 0.0, 0.0, 0.0, 1.96299, 0.0, 6.53387, 0.0, 0.0,
                   3.36001, 1.96299},
                  3, 4);
    const idv ckid = {"Mo", "S", "S", "S"};
  ...
  // cartesian, with abl one type missing
  {
    ...
    const fMat ck({0.0, 0.0, 1.96299, 0.0, 9.89385, 0.0, 0.0, 6.53387, 0.0, 0.0,
                   3.36001, 1.96299},
                  3, 4);
    const idv ckid = {"S", "S", "S", "S"};
...
void test_io::test_readWp() {
  ...
  {
    // check bad file
    CPPUNIT_ASSERT_THROW(readWp(opath + "Layer_Matrix.dat"),
                         std::runtime_error);
    ...
    CPPUNIT_ASSERT_THROW(readWp(wpath + "wannier90.wout", {3, 4, 1, 0}),
                         std::invalid_argument);

    // check wbl duplicate entries
    CPPUNIT_ASSERT_THROW(readWp(wpath + "wannier90.wout", {0, 1, 1, 2}),
                         std::invalid_argument);

    // check wbl illegal entries
    CPPUNIT_ASSERT_THROW(readWp(wpath + "wannier90.wout", {0, 1, 2, 172645}),
                         std::invalid_argument);
  ...
  // no wbl, no P
  {
    ...
    const fMat ck(
        {0.482391,  7.078464,  -0.007794, 0.000192,  -0.423313, 1.965239,
         -0.007638, 10.308024, -0.023286, -0.553955, 2.917203,  2.264988,
         0.038522,  2.946627,  1.315318,  0.003413,  9.467770,  0.644471,
         -1.139898, 0.450749,  -0.680885, -0.016440, 0.432696,  1.340404,
         0.551736,  9.443958,  -0.298955, -1.148999, 2.984042,  -0.638365,
         0.113523,  6.162493,  -0.105254, 3.475106,  1.740480,  0.023430,
         -0.538264, 9.432773,  -0.297512, -0.014971, 1.735985,  -0.031433,
         -0.552809, 6.967889,  -0.346652, -0.033350, 3.788433,  1.960502,
         -0.007120, 8.157331,  1.957693,  -0.031401, 8.083511,  2.019429,
         1.156762,  0.446536,  -0.657528, -0.057934, 1.731305,  0.006021,
         -1.700063, 8.081807,  -0.995361, -0.061165, 6.746088,  0.445236},
        3, 22);

    const fMat ckspread(
        {1.98218953, 1.36059869, 1.41498919, 1.92502688, 1.98677354, 1.89215823,
         1.86081759, 1.94003709, 1.86943488, 1.98311332, 1.59689773, 2.37554007,
         1.87489639, 2.14892524, 1.91210902, 1.51071199, 2.14108509, 3.73373851,
         1.86329425, 2.18784727, 3.56399432, 2.31241476},
        1, 22);
  ...
  // with wbl
  {
    ...
    const fMat ck(
        {0.000192,  -0.423313, 1.965239,  -0.007638, 10.308024, -0.023286,
         -0.553955, 2.917203,  2.264988,  0.038522,  2.946627,  1.315318,
         -1.139898, 0.450749,  -0.680885, -0.016440, 0.432696,  1.340404,
         0.551736,  9.443958,  -0.298955, -1.148999, 2.984042,  -0.638365,
         0.113523,  6.162493,  -0.105254, 3.475106,  1.740480,  0.023430,
         -0.538264, 9.432773,  -0.297512, -0.552809, 6.967889,  -0.346652,
         -0.033350, 3.788433,  1.960502,  -0.007120, 8.157331,  1.957693,
         -0.031401, 8.083511,  2.019429,  1.156762,  0.446536,  -0.657528,
         -0.057934, 1.731305,  0.006021,  -1.700063, 8.081807,  -0.995361,
...
void test_io::test_readHr() {
  ...
  {
    // check bad file
    CPPUNIT_ASSERT_THROW(readHr(wpath + "wannier90_hr.dat.cut"),
                         std::runtime_error);
    ...
    // check wbl not sorted
    CPPUNIT_ASSERT_THROW(readHr(wpath + "wannier90_hr.dat", {3, 4, 1, 0}),
                         std::invalid_argument);

    // check wbl duplicate entries
    CPPUNIT_ASSERT_THROW(readHr(wpath + "wannier90_hr.dat", {0, 1, 1, 2}),
                         std::invalid_argument);

    // check wbl illegal entries
    CPPUNIT_ASSERT_THROW(readHr(wpath + "wannier90_hr.dat", {0, 1, 2, 172645}),
                         std::invalid_argument);
...
void test_io::test_readXYZ() {
  ...
  {
    // check bad file
    CPPUNIT_ASSERT_THROW(readXYZ(wpath + "wannier90_r.dat.cut"),
                         std::runtime_error);
    ...
    // check wbl not sorted
    CPPUNIT_ASSERT_THROW(readXYZ(wpath + "wannier90_r.dat", 0, {3, 4, 1, 0}),
                         std::invalid_argument);

    // check wbl duplicate entries
    CPPUNIT_ASSERT_THROW(readXYZ(wpath + "wannier90_r.dat", 0, {0, 1, 1, 2}),
                         std::invalid_argument);

    // check wbl illegal entries
    CPPUNIT_ASSERT_THROW(
        readXYZ(wpath + "wannier90_r.dat", 0, {0, 1, 2, 172645}),
        std::invalid_argument);
...
void test_io::test_readWannierTransf() {
  ...
  // inas_mono
  {
    CPPUNIT_ASSERT_NO_THROW(
        readWannierTransf(wpath + "inas_mono/wannier90_u.mat"));
...
CppUnit::Test* test_io::suite() {
  ...
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_readEf", &test_io::test_readEf));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_genr", &test_io::test_genr));
  suite->addTest(new CppUnit::TestCaller<test_io>(
      "test_readLayerMatrix", &test_io::test_readLayerMatrix));
  suite->addTest(new CppUnit::TestCaller<test_io>("test_printOmf",
                                                  &test_io::test_printOmf));
  suite->addTest(new CppUnit::TestCaller<test_io>("test_printOlf",
                                                  &test_io::test_printOlf));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_readOmf", &test_io::test_readOmf));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_readOlf", &test_io::test_readOlf));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_hrDim", &test_io::test_hrDim));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_readB", &test_io::test_readB));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_readAp", &test_io::test_readAp));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_readWp", &test_io::test_readWp));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_readHr", &test_io::test_readHr));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_readAMN", &test_io::test_readAMN));
  //	suite->addTest(new CppUnit::TestCaller<test_io>(
  //		"test_readXYZ", &test_io::test_readXYZ));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_readEig", &test_io::test_readEig));
  suite->addTest(new CppUnit::TestCaller<test_io>(
      "test_readWannierTransf", &test_io::test_readWannierTransf));
  suite->addTest(
      new CppUnit::TestCaller<test_io>("test_readChk", &test_io::test_readChk));
