...
void test_mesh_all::test_mesh() {
  ...
  const auto tMat =
      rand<fMat>(M, std::accumulate(D.cbegin(), D.cend(), size_t(1),
...
void test_mesh_all::test_generators() {
  ...
  // 2D
  {
    ...
    {
      fMat ck(2, 0);
      ck.reserve(tMesh1.size());
      for (size_t j = 0; j != tMesh1.D(1); ++j)
        for (auto i = tMesh1.cbegin(0, {0, j}), e = tMesh1.cend(0, {0, j});
             i != e; ++i)
          ck.push_back(*i);

      for (auto i = tMesh1.base_.ccBegin(), e = tMesh1.base_.ccEnd(); i != e;
           ++i)
    ...
    }
    {
      ...
      for (size_t j = 0; j != tMesh1.D(0); ++j)
        for (auto i = tMesh1.cbegin(1, {j, 0}), e = tMesh1.cend(1, {j, 0});
             i != e; ++i)
          ck.push_back(*i);
...

  {
    ...
    {
      const auto tMesh =
          genMesh_cell(rho, B, lb, ub, maj, {false, false, false});
      tMesh.writeToFile("outp/tMesh_000__.bin");
    }
    {
      const auto tMesh =
          genMesh_cell(rho, B, lb, ub, maj, {true, false, false});
      tMesh.writeToFile("outp/tMesh_100.bin");
    }
    {
      const auto tMesh =
          genMesh_cell(rho, B, lb, ub, maj, {false, true, false});
      tMesh.writeToFile("outp/tMesh_010.bin");
    }
    {
      const auto tMesh =
          genMesh_cell(rho, B, lb, ub, maj, {false, false, true});
