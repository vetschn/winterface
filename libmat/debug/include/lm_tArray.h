 ...
 * Implements all features common to matrices, rows and columns, thus enabling
 * polymorphism between these types. Whenever treatment as an array is
 * sufficient, this class should be used over its children. However it is
 * important to realize that the main class is the matrix, since rows and
 * columns cannot exist independently and are hosted by a 'host' matrix where
 * they represent rows or columns of this matrix. However standalone rows or
 * columns are still possible in the form of an appropriately shaped matrix. The
 * number of rows is referred to as M, the number of columns as N, the linear
 * size, i.e. the total number of elements, as L. Since the underlying data is
 * arranged COLUMN-MAJOR (due to calls to FORTRAN routines), working over
 * columns should be preferred to over rows wherever possible. Template
 * arguments are: \n
 * - TT: the underlying type of this, either real (if this is an fArray) or
 * complex (if this is a cArray) \n
 * - FT: the real type \n
 * - CT: the complex type \n
 */
...
public:
  ...
  inline tItr dend(const size_t os = 0, const bool m = true) noexcept {
    return dbegin(os, m) +
           (m ? std::min(M() - os, N()) : std::min(M(), N() - os));
  }
  /** diagonal const_iterator to the first element
   * @param os diagonal offset, m ? offset along rows: offset along columns
   * @param m switch for offset aling rows(true), columns(false)
   */
  inline c_tItr dbegin(const size_t os = 0,
                       const bool m = true) const noexcept {
    assert((empty() && !os) || (m && os < M()) || (!m && os < N()));
   ...
   * @param m switch for offset aling rows(true), columns(false)
   */
  inline c_tItr cdbegin(const size_t os = 0,
                        const bool m = true) const noexcept {
    return dbegin(os, m);
  }
  /** past the end diagonal const_iterator
   ...
   * @param m switch for offset aling rows(true), columns(false)
   */
  inline cr_tItr rdbegin(const size_t os = 0,
                         const bool m = true) const noexcept {
   ...
   * @param m switch for offset aling rows(true), columns(false)
   */
  inline cr_tItr rdend(const size_t os = 0,
                       const bool m = true) const noexcept {
   ...
   * @param m switch for offset aling rows(true), columns(false)
   */
  inline cr_tItr crdbegin(const size_t os = 0,
                          const bool m = true) const noexcept {
   ...
   * @param m switch for offset aling rows(true), columns(false)
   */
  inline cr_tItr crdend(const size_t os = 0,
                        const bool m = true) const noexcept {
