 ...
 * This class can spawn so-called row or column iterators, which point to entire
 * rows or columns of the matrix. Each such iterator returns a row or column
 * upon dereferencing that is hosted by its 'host' matrix. These do not own
 * their own memory but rather manipulations on them change data in the 'host'
 * matrix. The api on rows, columns and matrices is as equivalent as can be due
 * all of them inheriting from lm_tArray. However, due to the column-major
 * arrangement of the underlying memory, this matrix class is specifically
 * designed as a sort of vector of columns. Working over columns is therefore to
 * be prioritized. However rows, columns and matrices may be intermixed in the
 * sense that one can be assigned to any of the others provided the total number
 * of elements is the same. Additionally, the matrix may have space for more
 * columns than it currently holds, analogous to std::vector. \n Template
 * arguments are: \n
 * - TT: the underlying type of this, either real (if this is an fMat) or
 * complex (if this is a cMat) \n
 * - FT: the real type \n
 * - CT: the complex type \n
 */
...
public:
  ...
  /** main constructor for a matrix of size MxN, memory is allocated but not
   * initialized
  ...
  /** constructor for passing preallocated data to the matrix, will be
   ...
   */
  tMat get(const size_t m, const size_t n, const size_t lm = NPOS__,
