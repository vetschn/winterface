 ...
 * private. It is only ever used by the functions rAt, rFront, rBack and
 * indirectly by the row iterator. The only purpose is as a window into a single
 * row of the hosting matrix. The reason this class works with a pointer to the
 * 'host' matrix rather than a raw pointer to the data directly, is that this
 * way iterator invalidation can be minimized. Template arguments are: \n
 * - TT: the underlying type of this, either real (if this is a fRow) or complex
 * (if this is a cRow) \n
 * - FT: the real type \n
 * - CT: the complex type\n
 */
...
public:
  ...
  friend class lm_tVecItr_b<
      lm_tMat<TT, FT, CT>, lm_tRow<TT, FT, CT>,
      lm_c_tVecItr<lm_tMat<TT, FT, CT>, lm_tRow<TT, FT, CT>>,
      lm_tVecItr<lm_tMat<TT, FT, CT>, lm_tRow<TT, FT, CT>>>;
  ...
  friend class lm_r_tVecItr_b<
      lm_tMat<TT, FT, CT>, lm_tRow<TT, FT, CT>,
      lm_cr_tVecItr<lm_tMat<TT, FT, CT>, lm_tRow<TT, FT, CT>>,
      lm_r_tVecItr<lm_tMat<TT, FT, CT>, lm_tRow<TT, FT, CT>>>;
...
private:
  ...
  inline explicit lm_tRow(const tMat *ptr = nullptr,
...
public:
  ...
  tMat *ptr() noexcept {
    return const_cast<tMat *>(static_cast<const lm_tRow *>(this)->ptr());
  }
