...
fMat abs(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! ceil assign for arrays
template <class TT, class FT, class CT>
lm_tArray<TT, FT, CT> &ceilEq(lm_tArray<TT, FT, CT> &inp) noexcept;
//! floor assign for arrays
template <class TT, class FT, class CT>
lm_tArray<TT, FT, CT> &floorEq(lm_tArray<TT, FT, CT> &inp) noexcept;
//! round assign for arrays
template <class TT, class FT, class CT>
lm_tArray<TT, FT, CT> &roundEq(lm_tArray<TT, FT, CT> &inp) noexcept;
//! sign assign for arrays
template <class TT, class FT, class CT>
lm_tArray<TT, FT, CT> &signEq(lm_tArray<TT, FT, CT> &inp) noexcept;
//! returns ceil for arrays
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> ceil(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! returns floor for arrays
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> floor(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! returns round for arrays
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> round(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! returns sign for arrays
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> sign(const lm_tArray<TT, FT, CT> &inp) noexcept;
...
env eGet(const fMat &Er, const fMat &Ei, const fMat &V,
 ...
 */
template <class MT>
MT rand(const size_t M, const size_t N, const RE__ rand_min = 0.0,
        const RE__ rand_max = 1.0) noexcept;
/** matrix filled with random numbers
 * @param S size struct
 * @param rand_min lower bound
 * @param rand_max upper bound
 */
template <class MT>
MT rand(const lm_size &S, const RE__ rand_min = 0.0,
        const RE__ rand_max = 1.0) noexcept;
 ...
 */
template <class MT>
MT randi(const size_t M, const size_t N, const long rand_min = 0,
         const long rand_max = 100) noexcept;
/** matrix filled with random integers
 * @param S size struct
 * @param rand_min lower bound
 * @param rand_max upper bound
 */
template <class MT>
MT randi(const lm_size &S, const long rand_min = 0,
         const long rand_max = 100) noexcept;
...
//! concatenate along rows [lhs;rhs]
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> mcat(const lm_tRow<TT, FT, CT> &lhs,
                         const lm_tMat<TT, FT, CT> &rhs) noexcept;
//! concatenate along rows [lhs;rhs]
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> mcat(const lm_tRow<TT, FT, CT> &lhs,
...
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> ncat(const lm_tArray<TT, FT, CT> &lhs,
...
template <class TT, class FT, class CT>
TT sum(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! product functional for arrays
template <class TT, class FT, class CT>
TT prod(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! min functional for arrays
template <class TT, class FT, class CT>
TT min(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! max functional for arrays
template <class TT, class FT, class CT>
TT max(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! mean functional for arrays
template <class TT, class FT, class CT>
TT mean(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! standard deviation functional for arrays
template <class TT, class FT, class CT>
FT stdd(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! standard deviation functional for arrays given a mean value
template <class TT, class FT, class CT>
FT stdd(const lm_tArray<TT, FT, CT> &inp, const TT &m) noexcept;
...
template <class TT, class FT, class CT>
FT norm(const lm_tArray<TT, FT, CT> &inp) noexcept;
//! trace
template <class TT, class FT, class CT>
TT trace(const lm_tMat<TT, FT, CT> &inp) noexcept;
//! determinante
template <class TT, class FT, class CT>
TT det(lm_tMat<TT, FT, CT> inp) noexcept;
...
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> mdot(const lm_tMat<TT, FT, CT> &lhs,
...
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> ndot(const lm_tMat<TT, FT, CT> &lhs,
                         const fMat &rhs) noexcept;
...
//! find dot product without adjoint of the columns of a matrix with a real
//! matrix
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> mdotu(const lm_tMat<TT, FT, CT> &lhs,
                          const fMat &rhs) noexcept;
//! find dot product without adjoint of the columns of a matrix with a complex
//! matrix
template <class TT, class FT, class CT>
...
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> ndotu(const lm_tMat<TT, FT, CT> &lhs,
                          const fMat &rhs) noexcept;
//! find dot product without adjoint of the rows of a matrix with a complex
//! matrix
template <class TT, class FT, class CT>
...
/** get the diagonal of a matrix or generate a matrix with a vector on the
 * diagonal
 * @param inp the matrix to extract the diagonal OR the row/col to use as the
 * diagonal in a matrix
 * @param os offset, m ? to the right: to the left
 * @param m offset switch
 */
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> diag(const lm_tArray<TT, FT, CT> &inp, const size_t os = 0,
                         const bool m = true) noexcept;
