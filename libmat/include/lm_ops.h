 ...
 * Instanced in file 'lm_fn.h'. Thread safety demands multiple stacks in
 * parallel regions.
 */
namespace hide {
#ifdef _OPENMP
extern std::vector<std::stack<RE__>>
    mtol_st_;  //!< stacks of stored mtol_ values
#else
...
#ifdef _OPENMP
  hide::mtol_st_[omp_get_thread_num()].push(
      tol);  // push tol to top of nth stack
#else
...
#else
  if (mtol_depth() > 1) hide::mtol_st_.pop();  // remove top element
#endif
}
...
inline void spawn_mtol_stacks(const size_t N) noexcept {
  ...
  for (auto i = hide::mtol_st_.begin() + 1, e = hide::mtol_st_.end(); i != e;
       ++i)
...
inline RE__ mtol() noexcept { return hide::mtol_; }
#endif
}  // namespace lm__
 ...
 * Template arguments are either real or complex. In comparisons the imaginary
 * parts are ignored.
 */
namespace ops {
...
//! strict cast to bool
template <class T1>
bool bool_s(const T1& inp) noexcept {
  return std::real(inp);
}
//! strict logical NOT
template <class T1>
bool not_s(const T1& inp) noexcept {
  return !std::real(inp);
}
//! strict logical AND
template <class T1, class T2>
bool and_s(const T1& lhs, const T2& rhs) noexcept {
  return std::real(lhs) && std::real(rhs);
}
//! strict logical OR
template <class T1, class T2>
bool or_s(const T1& lhs, const T2& rhs) noexcept {
  return std::real(lhs) || std::real(rhs);
}
...
template <class T1>
bool z_s(const T1& inp) noexcept {
  return not_s(inp);
}
//! strict comparison to not 0
template <class T1>
bool nz_s(const T1& inp) noexcept {
  return bool_s(inp);
}
//! strict equality
template <class T1, class T2>
bool eq_s(const T1& lhs, const T2& rhs) noexcept {
  return lhs == rhs;
}
//! strict inequality
template <class T1, class T2>
bool neq_s(const T1& lhs, const T2& rhs) noexcept {
  return lhs != rhs;
}
//! strict >=
template <class T1, class T2>
bool geq_s(const T1& lhs, const T2& rhs) noexcept {
  return std::real(lhs) >= std::real(rhs);
}
//! strict <=
template <class T1, class T2>
bool leq_s(const T1& lhs, const T2& rhs) noexcept {
  return std::real(lhs) <= std::real(rhs);
}
//! strict >
template <class T1, class T2>
bool gt_s(const T1& lhs, const T2& rhs) noexcept {
  return std::real(lhs) > std::real(rhs);
}
//! strict <
template <class T1, class T2>
bool lt_s(const T1& lhs, const T2& rhs) noexcept {
  return std::real(lhs) < std::real(rhs);
}
...
template <class T1>
bool z(const T1& inp) noexcept {
  return std::abs(inp) <= mtol();
}
//! tolerant comparison to not 0
template <class T1>
bool nz(const T1& inp) noexcept {
  return std::abs(inp) > mtol();
}
//! tolerant equality
template <class T1, class T2>
bool eq(const T1& lhs, const T2& rhs) noexcept {
  return std::abs(lhs - rhs) <= mtol();
}
//! tolerant inequality
template <class T1, class T2>
bool neq(const T1& lhs, const T2& rhs) noexcept {
  return std::abs(lhs - rhs) > mtol();
}
//! tolerant >=
template <class T1, class T2>
bool geq(const T1& lhs, const T2& rhs) noexcept {
  return std::real(rhs) - std::real(lhs) <= mtol();
}
//! tolerant <=
template <class T1, class T2>
bool leq(const T1& lhs, const T2& rhs) noexcept {
  return std::real(lhs) - std::real(rhs) <= mtol();
}
//! tolerant >
template <class T1, class T2>
bool gt(const T1& lhs, const T2& rhs) noexcept {
  return std::real(lhs) - std::real(rhs) > mtol();
}
//! tolerant <
template <class T1, class T2>
bool lt(const T1& lhs, const T2& rhs) noexcept {
  return std::real(rhs) - std::real(lhs) > mtol();
}
...
template <class T1>
bool bool_(const T1& inp) noexcept {
  return nz(inp);
}
//! tolerant logical NOT
template <class T1>
bool not_(const T1& inp) noexcept {
  return z(inp);
}
//! tolerant logical AND
template <class T1, class T2>
bool and_(const T1& lhs, const T2& rhs) noexcept {
  return nz(std::real(lhs)) && nz(std::real(rhs));
}
//! tolerant logical OR
template <class T1, class T2>
bool or_(const T1& lhs, const T2& rhs) noexcept {
  return nz(std::real(lhs)) || nz(std::real(rhs));
}
#else
 ...
 */
//! assign real to real
inline RE__& assign(RE__& lhs, const RE__ rhs) noexcept { return lhs = rhs; }
...
inline CPX__& assign(CPX__& lhs, const RE__& rhs) noexcept { return lhs = rhs; }

/* assignment equals operators
 */
//! add assign real to real
inline RE__& plusEq(RE__& lhs, const RE__ rhs) noexcept { return lhs += rhs; }
//! subtract assign real to real
inline RE__& minusEq(RE__& lhs, const RE__ rhs) noexcept { return lhs -= rhs; }
//! produce assign real to real
inline RE__& prodEq(RE__& lhs, const RE__ rhs) noexcept { return lhs *= rhs; }
//! divide assign real to real
inline RE__& divEq(RE__& lhs, const RE__ rhs) noexcept { return lhs /= rhs; }
...
inline RE__& divEq(RE__& lhs, const CPX__& rhs) noexcept {
  return lhs *= (std::real(rhs) / (std::real(rhs) * std::real(rhs) +
                                   std::imag(rhs) * std::imag(rhs)));
}
...
inline CPX__& plusEq(CPX__& lhs, const RE__ rhs) noexcept { return lhs += rhs; }
//! subtract assign real to complex
inline CPX__& minusEq(CPX__& lhs, const RE__ rhs) noexcept {
  return lhs -= rhs;
}
//! produce assign real to complex
inline CPX__& prodEq(CPX__& lhs, const RE__ rhs) noexcept { return lhs *= rhs; }
//! divide assign real to complex
inline CPX__& divEq(CPX__& lhs, const RE__ rhs) noexcept { return lhs /= rhs; }
...
#else
//! real ceil function
inline RE__ ceil(const RE__ inp) noexcept { return std::ceil(inp); }
//! real floor function
inline RE__ floor(const RE__ inp) noexcept { return std::floor(inp); }
//! real sign function
inline RE__ sign(const RE__ inp) noexcept {
...
inline RE__ round(const RE__ inp) noexcept { return std::round(inp); }
//! (tolerant) complex round function
