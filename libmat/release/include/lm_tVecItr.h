...
/**
 * Base class for vector iterators. I.e. row and column iterators. These
 * are 'stashing iterators', i.e. iterators that return a reference to a
 * member element, or in other words: they themselves contain what they
 * are pointing to. Therefore std::reverse_iterator does not work with
 * these and manual implementations of reverse iterators are provided.
 ...
 * and *b are bound to the same object. The reason is again the
 * reference of operator* to a member item. But since these items are a
 * tVec which have no substance in themselves, but are wrappers for a
 * specific row or column in a matrix. So *a and *b are bound to
 * different wrappers that point to the same object and thus the
 * condition can be regarded as fulfilled. \n \n In fact tVecs are
 * themselves almost iterators but we need certain operators such as
 * operator++ to act on the section of the matrix they point to as if
 * they were a matrix themselves and at the same time we need it to work
 * in the way an iterator would. The classes in this file thus provide
 * the iterator interface for the matrix interfaces in lm_tCol and
 * lm_tRow. The only way to make this work is for the vector iterator
 * classes to hold the vector it points to as a member and thus we end
 * up returning references to a member item. In all other ways these can
 * be considered standard iterators. \n Template arguments are: \n
 * - MT: 'host' matrix type
