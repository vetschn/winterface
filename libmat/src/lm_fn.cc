...
#ifdef _OPENMP
std::vector<std::stack<RE__>>
    lm__::hide::mtol_st_(1, std::stack<RE__>(std::deque<RE__>{MTOL__}));
#else
...
template <class MT> MT lm__::ones(const size_t M) noexcept {
  return ones<MT>(M, M);
}
template <> fMat lm__::ones(const size_t M, const size_t N) noexcept {
  fMat res(M, N);
  std::fill(res.begin(), res.end(), RE__(1.0));
  return res;
}
template <> cMat lm__::ones(const size_t M, const size_t N) noexcept {
  cMat res(M, N);
...
template <> fMat lm__::ones(const lm_size &S) noexcept {
  return ones<fMat>(S.M, S.N);
}
template <> cMat lm__::ones(const lm_size &S) noexcept {
  return ones<cMat>(S.M, S.N);
}
template <class MT> MT lm__::zeros(const size_t M) noexcept {
  return zeros<MT>(M, M);
}
template <> fMat lm__::zeros(const size_t M, const size_t N) noexcept {
  fMat res(M, N);
  std::fill(res.begin(), res.end(), RE__(0.0));
  return res;
}
template <> cMat lm__::zeros(const size_t M, const size_t N) noexcept {
  cMat res(M, N);
...
template <> fMat lm__::zeros(const lm_size &S) noexcept {
  return zeros<fMat>(S.M, S.N);
}
template <> cMat lm__::zeros(const lm_size &S) noexcept {
  return zeros<cMat>(S.M, S.N);
}
template <class MT> MT lm__::eye(const size_t M) noexcept {
  return eye<MT>(M, M);
}
template <> fMat lm__::eye(const size_t M, const size_t N) noexcept {
  auto res = zeros<fMat>(M, N);
  for (auto i = res.dbegin(), e = res.dend(); i != e; ++i)
    *i = RE__(1.0);
  return res;
}
template <> cMat lm__::eye(const size_t M, const size_t N) noexcept {
...
template <> fMat lm__::eye(const lm_size &S) noexcept {
  return eye<fMat>(S.M, S.N);
}
template <> cMat lm__::eye(const lm_size &S) noexcept {
  return eye<cMat>(S.M, S.N);
}
template <> fMat lm__::rId(const size_t N, const size_t i) noexcept {
  assert(i < N);
  auto res = zeros<fMat>(1, N);
  res[i] = RE__(1.0);
  return res;
}
template <> cMat lm__::rId(const size_t N, const size_t i) noexcept {
  assert(i < N);
  auto res = zeros<cMat>(1, N);
  res[i] = CPX__(1.0);
  return res;
}
template <> fMat lm__::cId(const size_t M, const size_t i) noexcept {
  assert(i < M);
  auto res = zeros<fMat>(M, 1);
  res[i] = RE__(1.0);
  return res;
}
template <> cMat lm__::cId(const size_t M, const size_t i) noexcept {
  assert(i < M);
  auto res = zeros<cMat>(M, 1);
  res[i] = CPX__(1.0);
  return res;
}
template <class MT> MT lm__::rand(const size_t M) noexcept {
...
template <> fMat lm__::rand(const fMat &l, const fMat &u) noexcept {
...
template <> cMat lm__::rand(const fMat &l, const fMat &u) noexcept {
  assert(msize(l) == msize(u));
  assert(all(l.leq(u)));
...
template <class MT> MT lm__::randi(const size_t M) noexcept {
...
template <> fMat lm__::randi(const fMat &l, const fMat &u) noexcept {
  set_mtol(0.0);
  assert(msize(l) == msize(u));
  assert(l == (round(l)));
  assert(u == (round(u)));
...
template <> cMat lm__::randi(const fMat &l, const fMat &u) noexcept {
  set_mtol(0.0);
  assert(msize(l) == msize(u));
  assert(l == (round(l)));
  assert(u == (round(u)));
...
lm_tMat<TT, FT, CT> lm__::complement(lm_tMat<TT, FT, CT> inp) noexcept {
  ...
  if (inp.empty() || inp.square())
    return lm_tMat<TT, FT, CT>(inp.M(), 0);
...
lm_tMat<TT, FT, CT> lm__::msum(const lm_tMat<TT, FT, CT> &inp) noexcept {
  if (inp.empty())
    return lm_tMat<TT, FT, CT>(0, inp.N());
...
mnp<TT, FT, CT> lm__::mmin(const lm_tMat<TT, FT, CT> &inp) noexcept {
  ...
  for (auto ii = inp.cBegin(), ie = inp.cEnd(); ii != ie; ++ii, ++ri) {
    const auto p =
        std::min_element(ii->begin(), ii->end(), [](const TT i, const TT j) {
          return ops::lt_s(i, j);
        });
    *ri = *p;
...
mnp<TT, FT, CT> lm__::mmax(const lm_tMat<TT, FT, CT> &inp) noexcept {
  ...
  for (auto ii = inp.cBegin(), ie = inp.cEnd(); ii != ie; ++ii, ++ri) {
    const auto p =
        std::max_element(ii->begin(), ii->end(), [](const TT i, const TT j) {
          return ops::lt_s(i, j);
        });
    *ri = *p;
    res.pos.push_back(p - ii->begin());
...
lm_tMat<TT, FT, CT> lm__::mdot(const lm_tMat<TT, FT, CT> &lhs,
  ...
  if (lhs.empty())
    return lm_tMat<TT, FT, CT>(0, lhs.N());
...
lm_tMat<TT, FT, CT> lm__::ndot(const lm_tMat<TT, FT, CT> &lhs,
  ...
  assert(msize(lhs) == msize(rhs));
  if (lhs.empty())
    return lm_tMat<TT, FT, CT>(lhs.M(), 0);
