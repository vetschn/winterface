...
lm_tMat<TT, FT, CT>& lm_tMat<TT, FT, CT>::rShift(const size_t m,
  ...
  if (N() * s > M() - m)
    memcpy(dst_, src_, (M() - m) * sizeof(TT));
  else
    memmove(dst_, src_, (M() - m) * sizeof(TT));
  ...
  if (N() > 1) {
    ...
    for (; src_ >= data() && size_t(dst_ - src_) > M();
         src_ -= M(), dst_ -= (M() + s))
...
lm_tMat<TT, FT, CT>& lm_tMat<TT, FT, CT>::cShift(const size_t n,
  ...
  // sufficient space
  if (s < N() - n)
    memmove(data() + M() * (n + s), data() + M() * n,
            M() * (N() - n) * sizeof(TT));
  else
    memcpy(data() + M() * (n + s), data() + M() * n,
           M() * (N() - n) * sizeof(TT));
...
lm_tMat<TT, FT, CT>& lm_tMat<TT, FT, CT>::set(const tMat& inp, const size_t m,
  ...
  auto s = inp.data();
  for (auto d = data() + n * M() + m, e = data() + (n + inp.N()) * M() + m;
       d != e; d += M(), s += inp.M())
...
lm_tMat<TT, FT, CT>& lm_tMat<TT, FT, CT>::rRm(
  ...
  auto i = data() + m[0];
  size_t move = 0;
  size_t max_move = m.size() * N() - k.back();
  size_t j = 0;
  while (move != max_move) {
...
lm_tMat<TT, FT, CT>& lm_tMat<TT, FT, CT>::cRm(
  ...
  for (i = 0; i != n.size() - 1; ++i)
    memmove(data() + (n[i] - i) * M(), data() + (n[i] + 1) * M(),
            (n[i + 1] - n[i] - 1) * M() * sizeof(TT));
  memmove(data() + (n[i] - i) * M(), data() + (n[i] + 1) * M(),
          (N() - n[i] - 1) * M() * sizeof(TT));
  N_ -= n.size();
...
lm_tMat<TT, FT, CT> lm_tMat<TT, FT, CT>::get(const size_t m, const size_t n,
                                             const size_t lm,
                                             const size_t ln) const noexcept {
  assert(m < M());
  assert(n < N());

  tMat res(M() - m < lm ? M() - m : lm, N() - n < ln ? N() - n : ln);
  auto dptr = res.data(), eptr = res.data() + res.L();
  auto sptr = data() + n * M() + m;
  while (dptr != eptr) {
...
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> lm_tMat<TT, FT, CT>::rGet(const size_t m,
                                              const size_t l) const noexcept {
  assert(!this->empty());
  assert(m + l <= M());
...
template <class TT, class FT, class CT>
lm_tMat<TT, FT, CT> lm_tMat<TT, FT, CT>::cGet(const size_t n,
                                              const size_t l) const noexcept {
  assert(!this->empty());
  assert(n + l <= N());
...
lm_tMat<TT, FT, CT> lm_tMat<TT, FT, CT>::lower() const noexcept {
  ...
  size_t l = M() - 1;
  auto ditr = res.data();
  auto sitr = data() + 1, e = data() + this->L();
  while (l && sitr < e) {
    memcpy(ditr, sitr, l * sizeof(TT));
    ditr += l;
    sitr += (M() + 1);
    ...
    if (!Lc)
      ...
      if (size_t(std::distance(i, e)) != Wc)
        throw(std::invalid_argument(
            "file \'" + fileName + "\', line " + std::to_string(Lc + 1) +
            ": found " + std::to_string(std::distance(i, e)) +
  ...
  for (size_t m = 0; m != M(); ++m) {
    ...
    if (size_t(std::distance(i, e)) < N())
      throw(std::invalid_argument("number of words on line " +
                                  ...
                                  std::to_string(std::distance(i, e)) +
