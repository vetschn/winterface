...
void test_tFn_mat_gen<CPX__, RE__, CPX__>::test_rand() {
  ...
  }
  {
    ...
    for (size_t i = 0; i != tMat1.L(); ++i) {
      CPPUNIT_ASSERT(std::real(tMat1[i]) >= l[i] &&
                     std::real(tMat1[i]) <= u[i]);
      CPPUNIT_ASSERT(std::imag(tMat1[i]) >= l[i] &&
                     std::imag(tMat1[i]) <= u[i]);
    }
  }
}
...
void test_tFn_mat_gen<CPX__, RE__, CPX__>::test_randi() {
  ...
  }
  {
    ...
    CPPUNIT_ASSERT_EQUAL(N, tMat1.N());
    for (size_t i = 0; i != tMat1.L(); ++i) {
      CPPUNIT_ASSERT_EQUAL(std::round(std::real(tMat1[i])),
                           std::real(tMat1[i]));
      CPPUNIT_ASSERT(std::real(tMat1[i]) >= l[i] &&
                     std::real(tMat1[i]) <= u[i]);
      CPPUNIT_ASSERT_EQUAL(std::round(std::imag(tMat1[i])),
                           std::imag(tMat1[i]));
      CPPUNIT_ASSERT(std::imag(tMat1[i]) >= l[i] &&
                     std::imag(tMat1[i]) <= u[i]);
    }
