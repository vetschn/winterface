...
void test_c_tVecItr_all<TT, FT, CT, VT>::test_ctor_assign() {
  ...
  // from cr_tVecItr
  {
    cr_tVecItr tItr1(const_cast<tMat *>(&tMat1), i);
    c_tVecItr tItr2(tItr1);
    CPPUNIT_ASSERT_EQUAL(const_cast<const tMat *>(tItr1.ptr()), tItr2.ptr());
...
template <class TT, class FT, class CT, class VT>
void test_c_tVecItr_all<TT, FT, CT, VT>::test_swap() {
  const size_t M = genRndST();
  const size_t N = genRndST();
  ...
  c_tVecItr tItr1(&tMat1, i1);
  c_tVecItr tItr2(&tMat2, i2);
...
template <class TT, class FT, class CT, class VT>
void test_c_tVecItr_all<TT, FT, CT, VT>::test_comparison() {
  const size_t M = genRndST();
  const size_t N = genRndST();
...
template <class TT, class FT, class CT, class VT>
void test_c_tVecItr_all<TT, FT, CT, VT>::test_arithmetic_difference() {
  const size_t M = genRndST();
  const size_t N = genRndST();
...
template <class TT, class FT, class CT, class VT>
CppUnit::Test *test_c_tVecItr_all<TT, FT, CT, VT>::suite() {
  CppUnit::TestSuite *suite = new CppUnit::TestSuite(test_id());

  suite->addTest(new CppUnit::TestCaller<test_c_tVecItr_all>(
      "test_ctor_assign",
      &test_c_tVecItr_all<TT, FT, CT, VT>::test_ctor_assign));
  suite->addTest(new CppUnit::TestCaller<test_c_tVecItr_all>(
      "test_swap", &test_c_tVecItr_all<TT, FT, CT, VT>::test_swap));
  suite->addTest(new CppUnit::TestCaller<test_c_tVecItr_all>(
      "test_comparison", &test_c_tVecItr_all<TT, FT, CT, VT>::test_comparison));
  suite->addTest(new CppUnit::TestCaller<test_c_tVecItr_all>(
      "test_dereference",
      &test_c_tVecItr_all<TT, FT, CT, VT>::test_dereference));
  suite->addTest(new CppUnit::TestCaller<test_c_tVecItr_all>(
      "test_arithmetic_difference",
      &test_c_tVecItr_all<TT, FT, CT, VT>::test_arithmetic_difference));
