...
template <ptrdiff_t s> void test_cpxItr_all<s>::test_c_ctor_assign() {
...
template <ptrdiff_t s> void test_cpxItr_all<s>::test_swap_distance() {
...
template <ptrdiff_t s> void test_cpxItr_all<s>::test_ctor_assign() {
  // lambda to generate random range
  auto gen = [](const size_t L) -> CPX__ * {
    CPX__ *res = new CPX__[L];
    rnd(res, L);
...
template <ptrdiff_t s> CppUnit::Test *test_cpxItr_all<s>::suite() {
