...
void test_tMat_comparison<TT, FT, CT>::test_eq_neq_leq_() {
  ...
  // <=,>=
  {
    auto tMat1 = rnd<tMat>(M, N, .5, 1.0);
    tMat1[0] = TT(.5);
    auto ftMat1 = rnd<fMat>(M, N, .0, .5);
    ftMat1[0] = FT(.5);
    CPPUNIT_ASSERT(all(tMat1.geq(ftMat1)));
    CPPUNIT_ASSERT(all(tMat1.geq(FT(.5))));

    auto tMat2 = rnd<tMat>(M, N, .0, .5);
    tMat2[0] = TT(.5);
    auto ftMat2 = rnd<fMat>(M, N, .5, 1.0);
    ftMat2[0] = FT(.5);
  ...
  }
  {
    ...
    auto tMat2 = rnd<tMat>(M, N, .0, .5);
    tMat2[0] = TT(.5);
    auto ctMat2 = rnd<cMat>(M, N, .5, 1.0);
    ctMat2[0] = CT(.5);
...
CppUnit::Test* test_tMat_comparison<TT, FT, CT>::suite() {
  ...
  suite->addTest(new CppUnit::TestCaller<test_tMat_comparison>(
      "test_size_function",
      &test_tMat_comparison<TT, FT, CT>::test_msize_function));
  suite->addTest(new CppUnit::TestCaller<test_tMat_comparison>(
      "test_operator_equal_unequal_",
      &test_tMat_comparison<TT, FT, CT>::test_operator_equal_unequal_));
  suite->addTest(new CppUnit::TestCaller<test_tMat_comparison>(
