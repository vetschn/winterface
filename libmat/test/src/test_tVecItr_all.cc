...
void test_tVecItr_all<TT, FT, CT, VT>::test_ctor_assign() {
...
template <class TT, class FT, class CT, class VT>
void test_tVecItr_all<TT, FT, CT, VT>::test_swap() {
  const size_t M = genRndST();
  const size_t N = genRndST();
  ...
  tVecItr tItr1(&tMat1, i1);
  tVecItr tItr2(&tMat2, i2);
...
template <class TT, class FT, class CT, class VT>
void test_tVecItr_all<TT, FT, CT, VT>::test_comparison() {
  const size_t M = genRndST();
  const size_t N = genRndST();
...
template <class TT, class FT, class CT, class VT>
void test_tVecItr_all<TT, FT, CT, VT>::test_arithmetic_difference() {
  const size_t M = genRndST();
  const size_t N = genRndST();
...
CppUnit::Test* test_tVecItr_all<TT, FT, CT, VT>::suite() {
  ...
  suite->addTest(new CppUnit::TestCaller<test_tVecItr_all>(
      "test_swap", &test_tVecItr_all<TT, FT, CT, VT>::test_swap));
  suite->addTest(new CppUnit::TestCaller<test_tVecItr_all>(
      "test_comparison", &test_tVecItr_all<TT, FT, CT, VT>::test_comparison));
  suite->addTest(new CppUnit::TestCaller<test_tVecItr_all>(
      "test_dereference", &test_tVecItr_all<TT, FT, CT, VT>::test_dereference));
  suite->addTest(new CppUnit::TestCaller<test_tVecItr_all>(
      "test_arithmetic_difference",
      &test_tVecItr_all<TT, FT, CT, VT>::test_arithmetic_difference));
