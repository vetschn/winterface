 ...
 public:
  ...
  struct printHelp_ : public virtual aux_parser::printHelp_,
    ...
    inline explicit printHelp_(const b_input &p, std::ostream &os)
        : aux_parser::printHelp_(p, os), ll_hbondss_input::printHelp_(p, os) {
      printHelpTuple_(
          os,
          std::make_tuple(
              TOPIC__("-b: FILENAMES"),
              HELP__("wout", p.wout, "wannier90 wout filename"),
              HELP__("hrdat", p.hrdat, "wannier90 hrdat filename"),
              HELP__("lattice_dat", p.lattice_dat, "OMEN lattice file"),
              HELP__("mat_par", p.mat_par, "OMEN material file"),
              HELP__("inprefix", p.inprefix,
                     "prefix for OMEN material, lattice and H_* files in "
                     "legacy mode"),
              HELP__("hr", p.hr, "input file for hr mode"),
              ...
              HELP__("r", p.r, "restriction vector"),
              HELP__("C", p.C, "expansion matrix for mode 1"),
              HELP__("R", p.R, "R vectors for mode 2"),
              HELP__("kpts", p.kpts, "list of k vectors in direct coordinates"),
              HELP__("k", p.k, "directly specified k points"),
              HELP__("strict_matching", p.strict_matching,
                     "allow strict type matching only"),
              HELP__("Nk", p.Nk, "number of points along k trace"),
              HELP__("rho_k", p.rho_k, "density of the mesh in the BZ"),
              ...
              HELP__("re", p.re, "skip imaginary parts"),
              HELP__("Nthreads", p.Nthreads,
                     "number of threads to use for bandstructure calculations "
                     "and\n"
                     "hamiltonian matrix generation\n"
                     "setting of 0 will set it to number of cores"),
              HELP__(
                  "dump_hamiltonians", p.dump_hamiltonians,
