...
void f_switch(const f_input& inp, std::ostream& os) {
  if (!inp.wout.empty()) {
    ...
    // write psc
    printPOSCAR(inp.prefix + inp.pscout + ".ap", B,
                inp.direct ? B.leftDivide(Ap.Ap()) : Ap.Ap(), Ap.id(), 1.0,
                inp.direct);
    printPOSCAR(inp.prefix + inp.pscout + ".wp", B,
                inp.direct ? B.leftDivide(Wp.Wp()) : Wp.Wp(), idv{Wp.N(), "H"},
                1.0, inp.direct);
  }
  ...
  if (!inp.lattice_dat.empty()) {
    ...
    printPOSCAR(inp.prefix + inp.pscout,
                ll_cell(D.B, D.B.leftDivide(D.Ap), D.id), inp.direct);
  }
}
