...
void l_switch(const l_input &inp, std::ostream &os) {
  ...
  // rotate basis
  if (inp.ROT != lm__::eye<fMat>(DIM__, DIM__) || inp.phi_x || inp.phi_y ||
      inp.phi_z) {
    const double phi_x = (M_PI / 180.0) * inp.phi_x;
    const double phi_y = (M_PI / 180.0) * inp.phi_y;
    const double phi_z = (M_PI / 180.0) * inp.phi_z;

    P.B = fMat({1.0, 0.0, 0.0, 0.0, std::cos(phi_x), std::sin(phi_x), 0.0,
                -std::sin(phi_x), std::cos(phi_x)},
               DIM__, DIM__)
              .prod(fMat({std::cos(phi_y), 0.0, -std::sin(phi_y), 0.0, 1.0, 0.0,
                          std::sin(phi_y), 0.0, std::cos(phi_y)},
                         DIM__, DIM__)
                        .prod(fMat({std::cos(phi_z), std::sin(phi_z), 0.0,
                                    -std::sin(phi_z), std::cos(phi_z), 0.0, 0.0,
                                    0.0, 1.0},
                                   DIM__, DIM__)
                                  .prod(inp.ROT.prod(P.B))));
  }
  ...
  if (!inp.vac.empty())
    for (size_t i = 0; i != P.B.M(); ++i) {
      ...
      const double f =
          (inp.vac.size() == P.Ap.M() ? inp.vac[i] : inp.vac.front()) /
          norm(P.B.cAt(i));
  ...
  if (inp.bond_factor) {
    ll_cell cell(std::move(P.B), P.Ap % 1.0, P.N, P.id);
    cell.merge(
        cell.getBonds(inp.bond_factor, genNNmat(!inp.r)).getBondCenters());
    P.N = cell.Ntype();
    P.B = cell.moveB();
    P.Ap = cell.moveAp();
    P.id = cell.id();
