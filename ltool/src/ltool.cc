  ...
  for (const auto &c : cmds)
    ...
    case 'l':
      try {
        ...
        if (direct_flag != -1)
          inp.direct = direct_flag;
        if (strip_flag != -1)
          inp.strip = strip_flag;
        if (!pscin.empty())
          inp.pscin = pscin;
        if (!pscout.empty())
          inp.pscout = pscout;
    ...
    case 'f':
      try {
        ...
        const auto df = [](const std::string &fileName) -> const char * {
          ...
          file.open(fileName);
          if (!file.good())
            throw(std::invalid_argument("open file \'" + std::string(fileName) +
                                        "\' failed"));
      ...
      try {
        if (!all_range_flag)
          ...
          for (const auto &i : W) {
            ...
            nR.resize(
                distance(nR.cBegin(), std::set_union(cR1.ccBegin(), cR1.ccEnd(),
