...
#define QUIT_1__(msg)                                \
  {                                                  \
    os << RED__ "ERROR: " << RESET__ << msg << "\n"; \
    return 0;                                        \
  }
#define QUIT_2__(inp, msg) \
  {                        \
    os << inp << "\n\n";   \
    QUIT_1__(msg);         \
  }
#define QUIT_SELECT__(_1, _2, NAME, ...) NAME
#define QUIT__(...) QUIT_SELECT__(__VA_ARGS__, QUIT_2__, QUIT_1__)(__VA_ARGS__)
...
int main(const int argc, char** argv) {
  ...
  while (true) {
    ...
    cmd = getopt_long(argc, argv, OPTS__, long_options, &options_index);
      ...
      switch (fnvHash(long_options[options_index].name)) {
        ...
        case "vac"_h:
          try {
            ...
            QUIT__("--vac: bad argument");
          }
          break;
        case "bond_factor"_h:
          try {
            bond_factor = std::stod(optarg);
          } catch (...) {
            QUIT__("--bond_factor: bad argument");
          }
          break;
      }
    ...
    if (cmd == -1) break;
    if (cmd == '?')
      QUIT__(std::string("-") + std::string(1, optopt) +
             std::string(": unknown option\n\n") + std::string(ERRMSG__));
    cmds.push_back(co_{cmd, optarg != nullptr ? optarg : ""});
  }

  // run commands
  for (const auto& c : cmds) switch (c.cmd) {
      case 'b':
        try {
          b_input inp =
              c.opt.empty() ? b_input() : parseFile<b_input>(c.opt, os);
      ...
      case 'l':
        try {
          ...
          l_input inp =
              c.opt.empty() ? l_input() : parseFile<l_input>(c.opt, os);
      ...
      case 'f':
        try {
          ...
          const auto df = [](const std::string& fileName) -> const char* {
            ...
            if (!line.empty() && std::isdigit(line[0])) return "omen";
          ...
          switch (fnvHash(df(c.opt))) {
            case "wannier"_h:
              ...
              break;
            case "omen"_h:
              inp.lattice_dat = c.opt;
              break;
            case "script"_h:
              inp = parseFile<f_input>(c.opt, os);
              break;
          }
      ...
      case 'p':
        try {
          p_input inp =
              c.opt.empty() ? p_input() : parseFile<p_input>(c.opt, os);
          if (direct_flag != -1) inp.direct = direct_flag;
          if (strip_flag != -1) inp.strip = strip_flag;
          if (!pscin.empty()) inp.pscin = pscin;
          if (!pscout.empty()) inp.pscout = pscout;
      ...
      case 'i':
        try {
          omen::writeInput(
              parseFile<h_input>(c.opt.empty() ? "winput" : c.opt, os), os);
        } catch (const std::exception& e) {
      ...
      case 'h':
        try {
          h_switch(parseFile<h_input>(c.opt.empty() ? "winput" : c.opt, os),
                   os);
        } catch (const std::exception& e) {
          QUIT__(h_input(), e.what());
        }
      ...
      case 'w':
        try {
          w_input inp =
              c.opt.empty() ? w_input() : aux::parseFile<w_input>(c.opt);
          if (!prefix.empty()) inp.prefix = prefix;
          if (verbosity != NPOS__) inp.verbosity = verbosity;
          if (!wbh.empty()) inp.wbh = wbh;
      ...
      case 'q':
        try {
          if (!all_range_flag)
            os << extractCell(c.opt.empty() ? WBH__ : c.opt).print(direct_flag)
               << "\n";
          else {
            ...
            for (const auto& i : W) {
              ...
              nR.resize(distance(
                  nR.cBegin(),
                  std::set_union(cR1.ccBegin(), cR1.ccEnd(), cR2.ccBegin(),
                                 cR2.ccEnd(), nR.cBegin())));
              nR.shrink_to_fit();
              RR[i.i2()] = std::move(nR);
            }
            ...
            os << "all range contents\n1.0\n"
               << T(W.cell().B()) << "\n"
               << W.cell().id() << "\n";
            for (const auto& i : RR) os << i.N() << " ";

            fMat Ap(W.dim(), 0);
            Ap.reserve(std::accumulate(
                RR.cbegin(), RR.cend(), size_t(0),
